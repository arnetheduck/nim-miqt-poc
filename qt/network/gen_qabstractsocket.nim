...
type cQAbstractSocketVTable = object
  ...
  connectToHost*: proc(vtbl, self: pointer, address: pointer, port: cushort, mode: cint): void {.cdecl, raises: [], gcsafe.}
...
proc miqt_exec_callback_cQAbstractSocket_errorWithQAbstractSocketSocketError_release(slot: int) {.exportc: "miqt_exec_callback_QAbstractSocket_errorWithQAbstractSocketSocketError_release".} =
...
type QAbstractSocketconnectToHostProc* = proc(self: QAbstractSocket, address: gen_qhostaddress_types.QHostAddress, port: cushort, mode: cint): void {.raises: [], gcsafe.}
...
proc miqt_exec_callback_cQAbstractSocket_connectToHost2(vtbl: pointer, self: pointer, address: pointer, port: cushort, mode: cint): void {.cdecl.} =
  let vtbl = cast[ptr QAbstractSocketVTable](vtbl)
  let self = QAbstractSocket(h: self)
  let slotval1 = gen_qhostaddress_types.QHostAddress(h: address)
  let slotval2 = port
  let slotval3 = cint(mode)
  vtbl[].connectToHost(self, slotval1, slotval2, slotval3)
...
proc create*(T: type gen_qabstractsocket_types.QAbstractSocket,
  ...
  if not isNil(vtbl.connectToHost):
    vtbl[].vtbl.connectToHost = miqt_exec_callback_cQAbstractSocket_connectToHost2
