...
proc create*(T: type QTimeZone, zoneId: seq[byte], offsetSeconds: cint, name: string, abbreviation: string, country: gen_qlocale.QLocaleCountry): QTimeZone =

  QTimeZone.init(fcQTimeZone_new6(struct_miqt_string(data: cast[cstring](if len(zoneId) == 0: nil else: unsafeAddr zoneId[0]), len: csize_t(len(zoneId))), offsetSeconds, struct_miqt_string(data: name, len: csize_t(len(name))), struct_miqt_string(data: abbreviation, len: csize_t(len(abbreviation))), cint(country)))
proc create*(T: type QTimeZone, zoneId: seq[byte], offsetSeconds: cint, name: string, abbreviation: string, country: gen_qlocale.QLocaleCountry, comment: string): QTimeZone =
...
proc availableTimeZoneIdsWithCountry*(_: type QTimeZone, country: gen_qlocale.QLocaleCountry): seq[seq[byte]] =
...
proc windowsIdToDefaultIanaId2*(_: type QTimeZone, windowsId: seq[byte], country: gen_qlocale.QLocaleCountry): seq[byte] =
...
proc windowsIdToIanaIds2*(_: type QTimeZone, windowsId: seq[byte], country: gen_qlocale.QLocaleCountry): seq[seq[byte]] =
