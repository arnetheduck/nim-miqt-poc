...
func (this *QQmlAbstractUrlInterceptor) Intercept(path *qt.QUrl, typeVal QQmlAbstractUrlInterceptor__DataType) *qt.QUrl {
	_goptr := qt.UnsafeNewQUrl(unsafe.Pointer(C.QQmlAbstractUrlInterceptor_intercept(this.h, (*C.QUrl)(path.UnsafePointer()), (C.int)(typeVal))))
...
func (this *QQmlAbstractUrlInterceptor) OnIntercept(slot func(path *qt.QUrl, typeVal QQmlAbstractUrlInterceptor__DataType) *qt.QUrl) {
...
func miqt_exec_callback_QQmlAbstractUrlInterceptor_intercept(self *C.QQmlAbstractUrlInterceptor, cb C.intptr_t, path *C.QUrl, typeVal C.int) *C.QUrl {
	gofunc, ok := cgo.Handle(cb).Value().(func(path *qt.QUrl, typeVal QQmlAbstractUrlInterceptor__DataType) *qt.QUrl)
	if !ok {
		panic("miqt: callback of non-callback type (heap corruption?)")
	}

	// Convert all CABI parameters to Go parameters
	slotval1 := qt.UnsafeNewQUrl(unsafe.Pointer(path))
