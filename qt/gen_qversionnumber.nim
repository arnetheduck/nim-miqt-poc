...
proc segmentAt*(self: QVersionNumber, index: cint): cint =

  fcQVersionNumber_segmentAt(self.h, index)

proc segmentCount*(self: QVersionNumber, ): cint =
...
proc fromString*(_: type QVersionNumber, string: string): QVersionNumber =

  QVersionNumber(h: fcQVersionNumber_fromString(struct_miqt_string(data: string, len: csize_t(len(string)))))

proc fromString22*(_: type QVersionNumber, string: string, suffixIndex: ptr cint): QVersionNumber =
