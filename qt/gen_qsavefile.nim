...
proc trUtf8*(_: type QSaveFile, s: cstring): string =
...
proc open*(self: QSaveFile, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool =
...
proc trUtf82*(_: type QSaveFile, s: cstring, c: cstring): string =
...
proc trUtf83*(_: type QSaveFile, s: cstring, c: cstring, n: cint): string =
...
proc callVirtualBase_open(self: QSaveFile, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool =


  fQSaveFile_virtualbase_open(self.h, cint(flags))

type QSaveFileopenBase* = proc(flags: gen_qiodevice.QIODeviceOpenModeFlag): bool
proc onopen*(self: QSaveFile, slot: proc(super: QSaveFileopenBase, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool) =
  # TODO check subclass
  type Cb = proc(super: QSaveFileopenBase, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
