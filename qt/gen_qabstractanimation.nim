...
proc fcQAnimationDriver_protectedbase_advanceAnimation1(self: pointer, timeStep: clonglong): void {.importc: "QAnimationDriver_protectedbase_advanceAnimation1".}
...
proc miqt_exec_callback_cQAnimationDriver_elapsed(vtbl: pointer, self: pointer): clonglong {.cdecl.} =
  let vtbl = cast[ptr QAnimationDriverVTable](vtbl)
  let self = QAnimationDriver(h: self)
  let virtualReturn = vtbl[].elapsed(self)
  virtualReturn
...
proc advanceAnimation*(self: gen_qabstractanimation_types.QAnimationDriver, timeStep: clonglong): void =
  fcQAnimationDriver_protectedbase_advanceAnimation1(self.h, timeStep)
