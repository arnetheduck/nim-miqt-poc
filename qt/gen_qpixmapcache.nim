...
proc find*(_: type QPixmapCache, key: string): gen_qpixmap.QPixmap =

  gen_qpixmap.QPixmap(h: fcQPixmapCache_find(struct_miqt_string(data: key, len: csize_t(len(key)))))

proc find2*(_: type QPixmapCache, key: string, pixmap: gen_qpixmap.QPixmap): bool =

  fcQPixmapCache_find2(struct_miqt_string(data: key, len: csize_t(len(key))), pixmap.h)

proc find3*(_: type QPixmapCache, key: string, pixmap: gen_qpixmap.QPixmap): bool =

  fcQPixmapCache_find3(struct_miqt_string(data: key, len: csize_t(len(key))), pixmap.h)

proc find4*(_: type QPixmapCache, key: QPixmapCacheKey, pixmap: gen_qpixmap.QPixmap): bool =
