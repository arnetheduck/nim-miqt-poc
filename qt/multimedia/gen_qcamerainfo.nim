...
func init*(T: type QCameraInfo, h: ptr cQCameraInfo): QCameraInfo =
  T(h: h)
proc create*(T: type QCameraInfo, ): QCameraInfo =

  QCameraInfo.init(fcQCameraInfo_new())
proc create*(T: type QCameraInfo, camera: gen_qcamera.QCamera): QCameraInfo =

  QCameraInfo.init(fcQCameraInfo_new2(camera.h))
proc create2*(T: type QCameraInfo, other: QCameraInfo): QCameraInfo =

  QCameraInfo.init(fcQCameraInfo_new3(other.h))
proc create*(T: type QCameraInfo, name: seq[byte]): QCameraInfo =

  QCameraInfo.init(fcQCameraInfo_new4(struct_miqt_string(data: cast[cstring](if len(name) == 0: nil else: unsafeAddr name[0]), len: csize_t(len(name)))))
proc operatorAssign*(self: QCameraInfo, other: QCameraInfo): void =

  fcQCameraInfo_operatorAssign(self.h, other.h)

proc operatorEqual*(self: QCameraInfo, other: QCameraInfo): bool =

  fcQCameraInfo_operatorEqual(self.h, other.h)

proc operatorNotEqual*(self: QCameraInfo, other: QCameraInfo): bool =

  fcQCameraInfo_operatorNotEqual(self.h, other.h)

proc isNull*(self: QCameraInfo, ): bool =

  fcQCameraInfo_isNull(self.h)

proc deviceName*(self: QCameraInfo, ): string =
...
proc description*(self: QCameraInfo, ): string =
...
proc position*(self: QCameraInfo, ): gen_qcamera.QCameraPosition =

  gen_qcamera.QCameraPosition(fcQCameraInfo_position(self.h))

proc orientation*(self: QCameraInfo, ): cint =

  fcQCameraInfo_orientation(self.h)

proc defaultCamera*(_: type QCameraInfo, ): QCameraInfo =

  QCameraInfo(h: fcQCameraInfo_defaultCamera())

proc availableCameras*(_: type QCameraInfo, ): seq[QCameraInfo] =

  var v_ma = fcQCameraInfo_availableCameras()
  var vx_ret = newSeq[QCameraInfo](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[pointer]](v_ma.data)
  for i in 0 ..< v_ma.len:
    vx_ret[i] = QCameraInfo(h: v_outCast[i])
  vx_ret

proc availableCameras1*(_: type QCameraInfo, position: gen_qcamera.QCameraPosition): seq[QCameraInfo] =
