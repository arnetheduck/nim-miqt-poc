...
proc trUtf8*(_: type QFile, s: cstring): string =
...
proc readLink*(self: QFile, ): string =
...
proc readLinkWithFileName*(_: type QFile, fileName: string): string =
...
proc link2*(_: type QFile, oldname: string, newName: string): bool =
...
proc open*(self: QFile, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool =

  fcQFile_open(self.h, cint(flags))

proc open3*(self: QFile, fd: cint, ioFlags: gen_qiodevice.QIODeviceOpenModeFlag): bool =
...
proc trUtf82*(_: type QFile, s: cstring, c: cstring): string =
...
proc trUtf83*(_: type QFile, s: cstring, c: cstring, n: cint): string =
...
proc open33*(self: QFile, fd: cint, ioFlags: gen_qiodevice.QIODeviceOpenModeFlag, handleFlags: gen_qfiledevice.QFileDeviceFileHandleFlag): bool =
...
proc callVirtualBase_open(self: QFile, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool =


  fQFile_virtualbase_open(self.h, cint(flags))

type QFileopenBase* = proc(flags: gen_qiodevice.QIODeviceOpenModeFlag): bool
proc onopen*(self: QFile, slot: proc(super: QFileopenBase, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool) =
  # TODO check subclass
  type Cb = proc(super: QFileopenBase, flags: gen_qiodevice.QIODeviceOpenModeFlag): bool
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
