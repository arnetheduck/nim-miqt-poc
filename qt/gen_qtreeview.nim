...
type QTreeVieweditProc* = proc(self: QTreeView, index: gen_qabstractitemmodel_types.QModelIndex, trigger: cint, event: gen_qcoreevent_types.QEvent): bool {.raises: [], gcsafe.}
...
proc QTreeViewviewOptions*(self: gen_qtreeview_types.QTreeView, ): gen_qstyleoption_types.QStyleOptionViewItem =
  gen_qstyleoption_types.QStyleOptionViewItem(h: fcQTreeView_virtualbase_viewOptions(self.h))

proc miqt_exec_callback_cQTreeView_viewOptions(vtbl: pointer, self: pointer): pointer {.cdecl.} =
  let vtbl = cast[ptr QTreeViewVTable](vtbl)
  let self = QTreeView(h: self)
...
proc miqt_exec_callback_cQTreeView_changeEvent(vtbl: pointer, self: pointer, param1: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QTreeViewVTable](vtbl)
  let self = QTreeView(h: self)
...
proc miqt_exec_callback_cQTreeView_nativeEvent(vtbl: pointer, self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QTreeViewVTable](vtbl)
  let self = QTreeView(h: self)
...
proc create*(T: type gen_qtreeview_types.QTreeView,
  ...
  if not isNil(vtbl.edit):
    vtbl[].vtbl.edit = miqt_exec_callback_cQTreeView_edit2
  ...
  if not isNil(vtbl.viewOptions):
    vtbl[].vtbl.viewOptions = miqt_exec_callback_cQTreeView_viewOptions
