...
proc create*(T: type gen_qwebframe_types.QWebHitTestResult, other: QWebHitTestResult): gen_qwebframe_types.QWebHitTestResult =
  gen_qwebframe_types.QWebHitTestResult.init(fcQWebHitTestResult_new2(other.h))

proc operatorAssign*(self: gen_qwebframe_types.QWebHitTestResult, other: QWebHitTestResult): void =
...
proc pos*(self: gen_qwebframe_types.QWebHitTestResult, ): QPoint =
  QPoint(h: fcQWebHitTestResult_pos(self.h))

proc boundingRect*(self: gen_qwebframe_types.QWebHitTestResult, ): QRect =
  QRect(h: fcQWebHitTestResult_boundingRect(self.h))

proc enclosingBlockElement*(self: gen_qwebframe_types.QWebHitTestResult, ): QWebElement =
  QWebElement(h: fcQWebHitTestResult_enclosingBlockElement(self.h))
...
proc linkUrl*(self: gen_qwebframe_types.QWebHitTestResult, ): QUrl =
  QUrl(h: fcQWebHitTestResult_linkUrl(self.h))

proc linkTitle*(self: gen_qwebframe_types.QWebHitTestResult, ): QUrl =
  QUrl(h: fcQWebHitTestResult_linkTitle(self.h))
...
proc linkTargetFrame*(self: gen_qwebframe_types.QWebHitTestResult, ): QWebFrame =
  QWebFrame(h: fcQWebHitTestResult_linkTargetFrame(self.h))

proc linkElement*(self: gen_qwebframe_types.QWebHitTestResult, ): QWebElement =
  QWebElement(h: fcQWebHitTestResult_linkElement(self.h))
...
proc imageUrl*(self: gen_qwebframe_types.QWebHitTestResult, ): QUrl =
  QUrl(h: fcQWebHitTestResult_imageUrl(self.h))

proc pixmap*(self: gen_qwebframe_types.QWebHitTestResult, ): QPixmap =
  QPixmap(h: fcQWebHitTestResult_pixmap(self.h))

proc mediaUrl*(self: gen_qwebframe_types.QWebHitTestResult, ): QUrl =
  QUrl(h: fcQWebHitTestResult_mediaUrl(self.h))
...
proc element*(self: gen_qwebframe_types.QWebHitTestResult, ): QWebElement =
  QWebElement(h: fcQWebHitTestResult_element(self.h))

proc frame*(self: gen_qwebframe_types.QWebHitTestResult, ): QWebFrame =
...
func init*(T: type gen_qwebframe_types.QWebFrame, h: ptr cQWebFrame): gen_qwebframe_types.QWebFrame =
  T(h: h)
proc metaObject*(self: gen_qwebframe_types.QWebFrame, ): QMetaObject =
  QMetaObject(h: fcQWebFrame_metaObject(self.h))
...
proc page*(self: gen_qwebframe_types.QWebFrame, ): QWebPage =
  QWebPage(h: fcQWebFrame_page(self.h))

proc load*(self: gen_qwebframe_types.QWebFrame, url: QUrl): void =
  fcQWebFrame_load(self.h, url.h)

proc load*(self: gen_qwebframe_types.QWebFrame, request: QNetworkRequest): void =
...
proc addToJavaScriptWindowObject*(self: gen_qwebframe_types.QWebFrame, name: string, objectVal: QObject): void =
...
proc setUrl*(self: gen_qwebframe_types.QWebFrame, url: QUrl): void =
  fcQWebFrame_setUrl(self.h, url.h)

proc url*(self: gen_qwebframe_types.QWebFrame, ): QUrl =
  QUrl(h: fcQWebFrame_url(self.h))

proc requestedUrl*(self: gen_qwebframe_types.QWebFrame, ): QUrl =
  QUrl(h: fcQWebFrame_requestedUrl(self.h))

proc baseUrl*(self: gen_qwebframe_types.QWebFrame, ): QUrl =
  QUrl(h: fcQWebFrame_baseUrl(self.h))

proc icon*(self: gen_qwebframe_types.QWebFrame, ): QIcon =
  QIcon(h: fcQWebFrame_icon(self.h))
...
proc parentFrame*(self: gen_qwebframe_types.QWebFrame, ): QWebFrame =
  QWebFrame(h: fcQWebFrame_parentFrame(self.h))

proc childFrames*(self: gen_qwebframe_types.QWebFrame, ): seq[QWebFrame] =
...
proc scrollBarGeometry*(self: gen_qwebframe_types.QWebFrame, orientation: cint): QRect =
  QRect(h: fcQWebFrame_scrollBarGeometry(self.h, cint(orientation)))

proc scroll*(self: gen_qwebframe_types.QWebFrame, param1: cint, param2: cint): void =
  fcQWebFrame_scroll(self.h, param1, param2)

proc scrollPosition*(self: gen_qwebframe_types.QWebFrame, ): QPoint =
  QPoint(h: fcQWebFrame_scrollPosition(self.h))

proc setScrollPosition*(self: gen_qwebframe_types.QWebFrame, pos: QPoint): void =
...
proc render*(self: gen_qwebframe_types.QWebFrame, param1: QPainter): void =
  fcQWebFrame_render(self.h, param1.h)

proc render*(self: gen_qwebframe_types.QWebFrame, param1: QPainter, layer: cint): void =
...
proc pos*(self: gen_qwebframe_types.QWebFrame, ): QPoint =
  QPoint(h: fcQWebFrame_pos(self.h))

proc geometry*(self: gen_qwebframe_types.QWebFrame, ): QRect =
  QRect(h: fcQWebFrame_geometry(self.h))

proc contentsSize*(self: gen_qwebframe_types.QWebFrame, ): QSize =
  QSize(h: fcQWebFrame_contentsSize(self.h))

proc documentElement*(self: gen_qwebframe_types.QWebFrame, ): QWebElement =
  QWebElement(h: fcQWebFrame_documentElement(self.h))

proc ownerElement*(self: gen_qwebframe_types.QWebFrame, ): QWebElement =
  QWebElement(h: fcQWebFrame_ownerElement(self.h))

proc findAllElements*(self: gen_qwebframe_types.QWebFrame, selectorQuery: string): QWebElementCollection =
  QWebElementCollection(h: fcQWebFrame_findAllElements(self.h, struct_miqt_string(data: selectorQuery, len: csize_t(len(selectorQuery)))))

proc findFirstElement*(self: gen_qwebframe_types.QWebFrame, selectorQuery: string): QWebElement =
  QWebElement(h: fcQWebFrame_findFirstElement(self.h, struct_miqt_string(data: selectorQuery, len: csize_t(len(selectorQuery)))))

proc hitTestContent*(self: gen_qwebframe_types.QWebFrame, pos: QPoint): QWebHitTestResult =
  QWebHitTestResult(h: fcQWebFrame_hitTestContent(self.h, pos.h))

proc event*(self: gen_qwebframe_types.QWebFrame, param1: QEvent): bool =
  fcQWebFrame_event(self.h, param1.h)

proc securityOrigin*(self: gen_qwebframe_types.QWebFrame, ): QWebSecurityOrigin =
  QWebSecurityOrigin(h: fcQWebFrame_securityOrigin(self.h))

proc evaluateJavaScript*(self: gen_qwebframe_types.QWebFrame, scriptSource: string): QVariant =
  QVariant(h: fcQWebFrame_evaluateJavaScript(self.h, struct_miqt_string(data: scriptSource, len: csize_t(len(scriptSource)))))

proc print*(self: gen_qwebframe_types.QWebFrame, printer: QPrinter): void =
...
proc urlChanged*(self: gen_qwebframe_types.QWebFrame, url: QUrl): void =
  fcQWebFrame_urlChanged(self.h, url.h)

type QWebFrameurlChangedSlot* = proc(url: QUrl)
...
proc contentsSizeChanged*(self: gen_qwebframe_types.QWebFrame, size: QSize): void =
  fcQWebFrame_contentsSizeChanged(self.h, size.h)

type QWebFramecontentsSizeChangedSlot* = proc(size: QSize)
...
proc load*(self: gen_qwebframe_types.QWebFrame, request: QNetworkRequest, operation: cint): void =
  fcQWebFrame_load2(self.h, request.h, cint(operation))

proc load*(self: gen_qwebframe_types.QWebFrame, request: QNetworkRequest, operation: cint, body: seq[byte]): void =
...
proc setHtml*(self: gen_qwebframe_types.QWebFrame, html: string, baseUrl: QUrl): void =
...
proc setContent*(self: gen_qwebframe_types.QWebFrame, data: seq[byte], mimeType: string, baseUrl: QUrl): void =
...
proc addToJavaScriptWindowObject*(self: gen_qwebframe_types.QWebFrame, name: string, objectVal: QObject, ownership: cint): void =
...
proc render*(self: gen_qwebframe_types.QWebFrame, param1: QPainter, clip: QRegion): void =
