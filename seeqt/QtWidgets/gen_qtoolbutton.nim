...
method nativeEvent*(self: VirtualQToolButton, eventType: seq[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
  QToolButtonnativeEvent(self[], eventType, message, resultVal)
proc miqt_exec_method_cQToolButton_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[VirtualQToolButton](cast[uint](vtbl) - uint(offsetOf(VirtualQToolButton, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
