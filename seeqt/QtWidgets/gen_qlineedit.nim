...
proc miqt_exec_method_cQLineEdit_keyReleaseEvent(vtbl: pointer, inst: pointer, event: pointer): void {.cdecl.} =
  let vtbl = cast[VirtualQLineEdit](cast[uint](vtbl) - uint(offsetOf(VirtualQLineEdit, vtbl)))
  let slotval1 = gen_qevent_types.QKeyEvent(h: event, owned: false)
...
method nativeEvent*(self: VirtualQLineEdit, eventType: seq[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
  QLineEditnativeEvent(self[], eventType, message, resultVal)
proc miqt_exec_method_cQLineEdit_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[VirtualQLineEdit](cast[uint](vtbl) - uint(offsetOf(VirtualQLineEdit, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc miqt_exec_method_cQLineEdit_timerEvent(vtbl: pointer, inst: pointer, event: pointer): void {.cdecl.} =
  let vtbl = cast[VirtualQLineEdit](cast[uint](vtbl) - uint(offsetOf(VirtualQLineEdit, vtbl)))
  let slotval1 = gen_qcoreevent_types.QTimerEvent(h: event, owned: false)
