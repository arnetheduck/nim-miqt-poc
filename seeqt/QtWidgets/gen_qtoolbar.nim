...
method nativeEvent*(self: VirtualQToolBar, eventType: seq[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
  QToolBarnativeEvent(self[], eventType, message, resultVal)
proc miqt_exec_method_cQToolBar_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[VirtualQToolBar](cast[uint](vtbl) - uint(offsetOf(VirtualQToolBar, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
