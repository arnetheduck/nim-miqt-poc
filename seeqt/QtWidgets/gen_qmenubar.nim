...
method enterEvent*(self: VirtualQMenuBar, event: gen_qevent_types.QEnterEvent): void {.base.} =
...
proc miqt_exec_method_cQMenuBar_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[VirtualQMenuBar](cast[uint](vtbl) - uint(offsetOf(VirtualQMenuBar, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
