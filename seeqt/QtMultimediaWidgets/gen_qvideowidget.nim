...
proc videoSink*(self: gen_qvideowidget_types.QVideoWidget, ): QVideoSink =
  QVideoSink(h: fcQVideoWidget_videoSink(self.h))
...
proc onaspectRatioModeChanged*(self: gen_qvideowidget_types.QVideoWidget, slot: QVideoWidgetaspectRatioModeChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fQVideoWidget_connect_aspectRatioModeChanged(self.h, cast[int](addr tmp[]))
...
proc miqt_exec_callback_QVideoWidget_focusOutEvent(self: ptr cQVideoWidget, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QVideoWidget_focusOutEvent ".} =
  ...
  nimfunc[](slotval1)
proc QVideoWidgetenterEvent*(self: gen_qvideowidget_types.QVideoWidget, event: QEnterEvent): void =
  fQVideoWidget_virtualbase_enterEvent(self.h, event.h)

type QVideoWidgetenterEventProc* = proc(event: QEnterEvent): void
