...
proc contains*(self: gen_qwebenginescriptcollection_types.QWebEngineScriptCollection, value: QWebEngineScript): bool =
  fcQWebEngineScriptCollection_contains(self.h, value.h)

proc find*(self: gen_qwebenginescriptcollection_types.QWebEngineScriptCollection, name: string): seq[QWebEngineScript] =
  var v_ma = fcQWebEngineScriptCollection_find(self.h, struct_miqt_string(data: name, len: csize_t(len(name))))
  var vx_ret = newSeq[QWebEngineScript](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[pointer]](v_ma.data)
  for i in 0 ..< v_ma.len:
    vx_ret[i] = QWebEngineScript(h: v_outCast[i])
  vx_ret

proc insert*(self: gen_qwebenginescriptcollection_types.QWebEngineScriptCollection, param1: QWebEngineScript): void =
  fcQWebEngineScriptCollection_insert(self.h, param1.h)

proc insert*(self: gen_qwebenginescriptcollection_types.QWebEngineScriptCollection, list: seq[QWebEngineScript]): void =
...
proc remove*(self: gen_qwebenginescriptcollection_types.QWebEngineScriptCollection, param1: QWebEngineScript): bool =
...
proc toList*(self: gen_qwebenginescriptcollection_types.QWebEngineScriptCollection, ): seq[QWebEngineScript] =
