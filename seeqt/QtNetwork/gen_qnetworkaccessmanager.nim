...
proc create*(T: type gen_qnetworkaccessmanager_types.QNetworkAccessManager, parent: QObject): gen_qnetworkaccessmanager_types.QNetworkAccessManager =
  gen_qnetworkaccessmanager_types.QNetworkAccessManager.init(fcQNetworkAccessManager_new2(parent.h))

proc metaObject*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): QMetaObject =
  QMetaObject(h: fcQNetworkAccessManager_metaObject(self.h))
...
proc proxy*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): QNetworkProxy =
  QNetworkProxy(h: fcQNetworkAccessManager_proxy(self.h))

proc setProxy*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, proxy: QNetworkProxy): void =
  fcQNetworkAccessManager_setProxy(self.h, proxy.h)

proc proxyFactory*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): QNetworkProxyFactory =
  QNetworkProxyFactory(h: fcQNetworkAccessManager_proxyFactory(self.h))

proc setProxyFactory*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, factory: QNetworkProxyFactory): void =
  fcQNetworkAccessManager_setProxyFactory(self.h, factory.h)

proc cache*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): QAbstractNetworkCache =
  QAbstractNetworkCache(h: fcQNetworkAccessManager_cache(self.h))

proc setCache*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, cache: QAbstractNetworkCache): void =
  fcQNetworkAccessManager_setCache(self.h, cache.h)

proc cookieJar*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): QNetworkCookieJar =
  QNetworkCookieJar(h: fcQNetworkAccessManager_cookieJar(self.h))

proc setCookieJar*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, cookieJar: QNetworkCookieJar): void =
...
proc addStrictTransportSecurityHosts*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, knownHosts: seq[QHstsPolicy]): void =
...
proc strictTransportSecurityHosts*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): seq[QHstsPolicy] =
  var v_ma = fcQNetworkAccessManager_strictTransportSecurityHosts(self.h)
  var vx_ret = newSeq[QHstsPolicy](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[pointer]](v_ma.data)
  for i in 0 ..< v_ma.len:
    vx_ret[i] = QHstsPolicy(h: v_outCast[i])
  vx_ret

proc head*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_head(self.h, request.h))

proc get*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_get(self.h, request.h))

proc post*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, data: QIODevice): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_post(self.h, request.h, data.h))

proc post*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, data: seq[byte]): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_post2(self.h, request.h, struct_miqt_string(data: cast[cstring](if len(data) == 0: nil else: unsafeAddr data[0]), len: csize_t(len(data)))))

proc put*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, data: QIODevice): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_put(self.h, request.h, data.h))

proc put*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, data: seq[byte]): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_put2(self.h, request.h, struct_miqt_string(data: cast[cstring](if len(data) == 0: nil else: unsafeAddr data[0]), len: csize_t(len(data)))))

proc deleteResource*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_deleteResource(self.h, request.h))

proc sendCustomRequest*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, verb: seq[byte]): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_sendCustomRequest(self.h, request.h, struct_miqt_string(data: cast[cstring](if len(verb) == 0: nil else: unsafeAddr verb[0]), len: csize_t(len(verb)))))

proc sendCustomRequest*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, verb: seq[byte], data: seq[byte]): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_sendCustomRequest2(self.h, request.h, struct_miqt_string(data: cast[cstring](if len(verb) == 0: nil else: unsafeAddr verb[0]), len: csize_t(len(verb))), struct_miqt_string(data: cast[cstring](if len(data) == 0: nil else: unsafeAddr data[0]), len: csize_t(len(data)))))

proc post*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, multiPart: QHttpMultiPart): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_post3(self.h, request.h, multiPart.h))

proc put*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, multiPart: QHttpMultiPart): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_put3(self.h, request.h, multiPart.h))

proc sendCustomRequest*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, verb: seq[byte], multiPart: QHttpMultiPart): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_sendCustomRequest3(self.h, request.h, struct_miqt_string(data: cast[cstring](if len(verb) == 0: nil else: unsafeAddr verb[0]), len: csize_t(len(verb))), multiPart.h))

proc setConfiguration*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, config: QNetworkConfiguration): void =
  fcQNetworkAccessManager_setConfiguration(self.h, config.h)

proc configuration*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): QNetworkConfiguration =
  QNetworkConfiguration(h: fcQNetworkAccessManager_configuration(self.h))

proc activeConfiguration*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, ): QNetworkConfiguration =
  QNetworkConfiguration(h: fcQNetworkAccessManager_activeConfiguration(self.h))
...
proc connectToHostEncrypted*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, hostName: string, port: cushort, sslConfiguration: QSslConfiguration, peerName: string): void =
...
proc proxyAuthenticationRequired*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, proxy: QNetworkProxy, authenticator: QAuthenticator): void =
  fcQNetworkAccessManager_proxyAuthenticationRequired(self.h, proxy.h, authenticator.h)

type QNetworkAccessManagerproxyAuthenticationRequiredSlot* = proc(proxy: QNetworkProxy, authenticator: QAuthenticator)
proc miqt_exec_callback_QNetworkAccessManager_proxyAuthenticationRequired(slot: int, proxy: pointer, authenticator: pointer) {.exportc.} =
  ...
  let slotval1 = QNetworkProxy(h: proxy)
...
proc authenticationRequired*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, reply: QNetworkReply, authenticator: QAuthenticator): void =
  fcQNetworkAccessManager_authenticationRequired(self.h, reply.h, authenticator.h)

type QNetworkAccessManagerauthenticationRequiredSlot* = proc(reply: QNetworkReply, authenticator: QAuthenticator)
...
proc finished*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, reply: QNetworkReply): void =
  fcQNetworkAccessManager_finished(self.h, reply.h)

type QNetworkAccessManagerfinishedSlot* = proc(reply: QNetworkReply)
...
proc encrypted*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, reply: QNetworkReply): void =
  fcQNetworkAccessManager_encrypted(self.h, reply.h)

type QNetworkAccessManagerencryptedSlot* = proc(reply: QNetworkReply)
...
proc sslErrors*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, reply: QNetworkReply, errors: seq[QSslError]): void =
...
type QNetworkAccessManagersslErrorsSlot* = proc(reply: QNetworkReply, errors: seq[QSslError])
proc miqt_exec_callback_QNetworkAccessManager_sslErrors(slot: int, reply: pointer, errors: struct_miqt_array) {.exportc.} =
  ...
  var verrorsx_ret = newSeq[QSslError](int(verrors_ma.len))
  let verrors_outCast = cast[ptr UncheckedArray[pointer]](verrors_ma.data)
  for i in 0 ..< verrors_ma.len:
    verrorsx_ret[i] = QSslError(h: verrors_outCast[i])
...
proc preSharedKeyAuthenticationRequired*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, reply: QNetworkReply, authenticator: QSslPreSharedKeyAuthenticator): void =
  fcQNetworkAccessManager_preSharedKeyAuthenticationRequired(self.h, reply.h, authenticator.h)

type QNetworkAccessManagerpreSharedKeyAuthenticationRequiredSlot* = proc(reply: QNetworkReply, authenticator: QSslPreSharedKeyAuthenticator)
proc miqt_exec_callback_QNetworkAccessManager_preSharedKeyAuthenticationRequired(slot: int, reply: pointer, authenticator: pointer) {.exportc.} =
  ...
  let slotval2 = QSslPreSharedKeyAuthenticator(h: authenticator)
...
proc sendCustomRequest*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: QNetworkRequest, verb: seq[byte], data: QIODevice): QNetworkReply =
  QNetworkReply(h: fcQNetworkAccessManager_sendCustomRequest32(self.h, request.h, struct_miqt_string(data: cast[cstring](if len(verb) == 0: nil else: unsafeAddr verb[0]), len: csize_t(len(verb))), data.h))
...
proc connectToHostEncrypted*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, hostName: string, port: cushort, sslConfiguration: QSslConfiguration): void =
...
proc miqt_exec_callback_QNetworkAccessManager_metacall(self: ptr cQNetworkAccessManager, slot: int, param1: cint, param2: cint, param3: pointer): cint {.exportc: "miqt_exec_callback_QNetworkAccessManager_metacall ".} =
  ...
  virtualReturn
proc QNetworkAccessManagercreateRequest*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, op: cint, request: QNetworkRequest, outgoingData: QIODevice): QNetworkReply =
  QNetworkReply(h: fQNetworkAccessManager_virtualbase_createRequest(self.h, cint(op), request.h, outgoingData.h))

type QNetworkAccessManagercreateRequestProc* = proc(op: cint, request: QNetworkRequest, outgoingData: QIODevice): QNetworkReply
...
proc miqt_exec_callback_QNetworkAccessManager_createRequest(self: ptr cQNetworkAccessManager, slot: int, op: cint, request: pointer, outgoingData: pointer): pointer {.exportc: "miqt_exec_callback_QNetworkAccessManager_createRequest ".} =
  ...
  let slotval3 = QIODevice(h: outgoingData)


  let virtualReturn = nimfunc[](slotval1, slotval2, slotval3 )

  virtualReturn.h
proc QNetworkAccessManagerevent*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, event: QEvent): bool =
  fQNetworkAccessManager_virtualbase_event(self.h, event.h)

type QNetworkAccessManagereventProc* = proc(event: QEvent): bool
...
proc miqt_exec_callback_QNetworkAccessManager_event(self: ptr cQNetworkAccessManager, slot: int, event: pointer): bool {.exportc: "miqt_exec_callback_QNetworkAccessManager_event ".} =
  ...
  virtualReturn
proc QNetworkAccessManagereventFilter*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, watched: QObject, event: QEvent): bool =
  fQNetworkAccessManager_virtualbase_eventFilter(self.h, watched.h, event.h)

type QNetworkAccessManagereventFilterProc* = proc(watched: QObject, event: QEvent): bool
...
proc miqt_exec_callback_QNetworkAccessManager_eventFilter(self: ptr cQNetworkAccessManager, slot: int, watched: pointer, event: pointer): bool {.exportc: "miqt_exec_callback_QNetworkAccessManager_eventFilter ".} =
  ...
  virtualReturn
proc QNetworkAccessManagertimerEvent*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, event: QTimerEvent): void =
  fQNetworkAccessManager_virtualbase_timerEvent(self.h, event.h)

type QNetworkAccessManagertimerEventProc* = proc(event: QTimerEvent): void
...
proc miqt_exec_callback_QNetworkAccessManager_timerEvent(self: ptr cQNetworkAccessManager, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QNetworkAccessManager_timerEvent ".} =
  ...
  nimfunc[](slotval1)
proc QNetworkAccessManagerchildEvent*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, event: QChildEvent): void =
  fQNetworkAccessManager_virtualbase_childEvent(self.h, event.h)

type QNetworkAccessManagerchildEventProc* = proc(event: QChildEvent): void
...
proc miqt_exec_callback_QNetworkAccessManager_childEvent(self: ptr cQNetworkAccessManager, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QNetworkAccessManager_childEvent ".} =
  ...
  nimfunc[](slotval1)
proc QNetworkAccessManagercustomEvent*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, event: QEvent): void =
  fQNetworkAccessManager_virtualbase_customEvent(self.h, event.h)

type QNetworkAccessManagercustomEventProc* = proc(event: QEvent): void
...
proc miqt_exec_callback_QNetworkAccessManager_customEvent(self: ptr cQNetworkAccessManager, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QNetworkAccessManager_customEvent ".} =
  ...
  nimfunc[](slotval1)
proc QNetworkAccessManagerconnectNotify*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, signal: QMetaMethod): void =
  fQNetworkAccessManager_virtualbase_connectNotify(self.h, signal.h)

type QNetworkAccessManagerconnectNotifyProc* = proc(signal: QMetaMethod): void
...
proc miqt_exec_callback_QNetworkAccessManager_connectNotify(self: ptr cQNetworkAccessManager, slot: int, signal: pointer): void {.exportc: "miqt_exec_callback_QNetworkAccessManager_connectNotify ".} =
  ...
  nimfunc[](slotval1)
proc QNetworkAccessManagerdisconnectNotify*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, signal: QMetaMethod): void =
  fQNetworkAccessManager_virtualbase_disconnectNotify(self.h, signal.h)

type QNetworkAccessManagerdisconnectNotifyProc* = proc(signal: QMetaMethod): void
