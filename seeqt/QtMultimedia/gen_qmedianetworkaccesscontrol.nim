...
proc fcQMediaNetworkAccessControl_currentConfiguration(self: pointer, ): cQNetworkConfiguration {.importc: "QMediaNetworkAccessControl_currentConfiguration".}
proc fcQMediaNetworkAccessControl_configurationChanged(self: pointer, configuration: ptr cQNetworkConfiguration): void {.importc: "QMediaNetworkAccessControl_configurationChanged".}
proc fcQMediaNetworkAccessControl_connect_configurationChanged(self: pointer, slot: int, callback: proc (slot: int, configuration: ptr cQNetworkConfiguration) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QMediaNetworkAccessControl_connect_configurationChanged".}
...
proc setConfigurations*(self: gen_qmedianetworkaccesscontrol_types.QMediaNetworkAccessControl, configuration: seq[QNetworkConfiguration]): void =
  var configuration_CArray = newSeq[cQNetworkConfiguration](len(configuration))
  for i in 0..<len(configuration):
    configuration_CArray[i] = configuration[i]
...
proc currentConfiguration*(self: gen_qmedianetworkaccesscontrol_types.QMediaNetworkAccessControl, ): QNetworkConfiguration =
TODO QNetworkConfiguration
proc configurationChanged*(self: gen_qmedianetworkaccesscontrol_types.QMediaNetworkAccessControl, configuration: ptr QNetworkConfiguration): void =
  fcQMediaNetworkAccessControl_configurationChanged(self.h, configuration)

type QMediaNetworkAccessControlconfigurationChangedSlot* = proc(configuration: ptr QNetworkConfiguration)
proc miqt_exec_callback_cQMediaNetworkAccessControl_configurationChanged(slot: int, configuration: ptr cQNetworkConfiguration) {.cdecl.} =
