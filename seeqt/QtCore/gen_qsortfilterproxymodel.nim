...
proc create*(T: type gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QObject): gen_qsortfilterproxymodel_types.QSortFilterProxyModel =
  gen_qsortfilterproxymodel_types.QSortFilterProxyModel.init(fcQSortFilterProxyModel_new2(parent.h))

proc metaObject*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, ): QMetaObject =
  QMetaObject(h: fcQSortFilterProxyModel_metaObject(self.h))
...
proc setSourceModel*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceModel: QAbstractItemModel): void =
  fcQSortFilterProxyModel_setSourceModel(self.h, sourceModel.h)

proc mapToSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, proxyIndex: QModelIndex): QModelIndex =
  QModelIndex(h: fcQSortFilterProxyModel_mapToSource(self.h, proxyIndex.h))

proc mapFromSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceIndex: QModelIndex): QModelIndex =
  QModelIndex(h: fcQSortFilterProxyModel_mapFromSource(self.h, sourceIndex.h))

proc mapSelectionToSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, proxySelection: QItemSelection): QItemSelection =
  QItemSelection(h: fcQSortFilterProxyModel_mapSelectionToSource(self.h, proxySelection.h))

proc mapSelectionFromSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceSelection: QItemSelection): QItemSelection =
  QItemSelection(h: fcQSortFilterProxyModel_mapSelectionFromSource(self.h, sourceSelection.h))

proc filterRegularExpression*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, ): QRegularExpression =
  QRegularExpression(h: fcQSortFilterProxyModel_filterRegularExpression(self.h))
...
proc setFilterRegularExpression*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, regularExpression: QRegularExpression): void =
...
proc index*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, column: cint, parent: QModelIndex): QModelIndex =
  QModelIndex(h: fcQSortFilterProxyModel_index(self.h, row, column, parent.h))

proc parent*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, child: QModelIndex): QModelIndex =
  QModelIndex(h: fcQSortFilterProxyModel_parent(self.h, child.h))

proc sibling*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, column: cint, idx: QModelIndex): QModelIndex =
  QModelIndex(h: fcQSortFilterProxyModel_sibling(self.h, row, column, idx.h))

proc rowCount*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): cint =
  fcQSortFilterProxyModel_rowCount(self.h, parent.h)

proc columnCount*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): cint =
  fcQSortFilterProxyModel_columnCount(self.h, parent.h)

proc hasChildren*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): bool =
  fcQSortFilterProxyModel_hasChildren(self.h, parent.h)

proc data*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex, role: cint): QVariant =
  QVariant(h: fcQSortFilterProxyModel_data(self.h, index.h, role))

proc setData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex, value: QVariant, role: cint): bool =
  fcQSortFilterProxyModel_setData(self.h, index.h, value.h, role)

proc headerData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, section: cint, orientation: cint, role: cint): QVariant =
  QVariant(h: fcQSortFilterProxyModel_headerData(self.h, section, cint(orientation), role))

proc setHeaderData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, section: cint, orientation: cint, value: QVariant, role: cint): bool =
  fcQSortFilterProxyModel_setHeaderData(self.h, section, cint(orientation), value.h, role)

proc mimeData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, indexes: seq[QModelIndex]): QMimeData =
  var indexes_CArray = newSeq[pointer](len(indexes))
  for i in 0..<len(indexes):
    indexes_CArray[i] = indexes[i].h

  QMimeData(h: fcQSortFilterProxyModel_mimeData(self.h, struct_miqt_array(len: csize_t(len(indexes)), data: if len(indexes) == 0: nil else: addr(indexes_CArray[0]))))

proc dropMimeData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, data: QMimeData, action: cint, row: cint, column: cint, parent: QModelIndex): bool =
  fcQSortFilterProxyModel_dropMimeData(self.h, data.h, cint(action), row, column, parent.h)

proc insertRows*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, count: cint, parent: QModelIndex): bool =
  fcQSortFilterProxyModel_insertRows(self.h, row, count, parent.h)

proc insertColumns*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, column: cint, count: cint, parent: QModelIndex): bool =
  fcQSortFilterProxyModel_insertColumns(self.h, column, count, parent.h)

proc removeRows*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, count: cint, parent: QModelIndex): bool =
  fcQSortFilterProxyModel_removeRows(self.h, row, count, parent.h)

proc removeColumns*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, column: cint, count: cint, parent: QModelIndex): bool =
  fcQSortFilterProxyModel_removeColumns(self.h, column, count, parent.h)

proc fetchMore*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): void =
  fcQSortFilterProxyModel_fetchMore(self.h, parent.h)

proc canFetchMore*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): bool =
  fcQSortFilterProxyModel_canFetchMore(self.h, parent.h)

proc flags*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): cint =
  cint(fcQSortFilterProxyModel_flags(self.h, index.h))

proc buddy*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): QModelIndex =
  QModelIndex(h: fcQSortFilterProxyModel_buddy(self.h, index.h))

proc match*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, start: QModelIndex, role: cint, value: QVariant, hits: cint, flags: cint): seq[QModelIndex] =
...
proc span*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): QSize =
  QSize(h: fcQSortFilterProxyModel_span(self.h, index.h))
...
proc miqt_exec_callback_QSortFilterProxyModel_metacall(self: ptr cQSortFilterProxyModel, slot: int, param1: cint, param2: cint, param3: pointer): cint {.exportc: "miqt_exec_callback_QSortFilterProxyModel_metacall ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelsetSourceModel*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceModel: QAbstractItemModel): void =
  fQSortFilterProxyModel_virtualbase_setSourceModel(self.h, sourceModel.h)

type QSortFilterProxyModelsetSourceModelProc* = proc(sourceModel: QAbstractItemModel): void
...
proc miqt_exec_callback_QSortFilterProxyModel_setSourceModel(self: ptr cQSortFilterProxyModel, slot: int, sourceModel: pointer): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_setSourceModel ".} =
  ...
  let slotval1 = QAbstractItemModel(h: sourceModel)


  nimfunc[](slotval1)
proc QSortFilterProxyModelmapToSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, proxyIndex: QModelIndex): QModelIndex =
  QModelIndex(h: fQSortFilterProxyModel_virtualbase_mapToSource(self.h, proxyIndex.h))

type QSortFilterProxyModelmapToSourceProc* = proc(proxyIndex: QModelIndex): QModelIndex
...
proc miqt_exec_callback_QSortFilterProxyModel_mapToSource(self: ptr cQSortFilterProxyModel, slot: int, proxyIndex: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_mapToSource ".} =
  ...
  let slotval1 = QModelIndex(h: proxyIndex)


  let virtualReturn = nimfunc[](slotval1 )

  virtualReturn.h
proc QSortFilterProxyModelmapFromSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceIndex: QModelIndex): QModelIndex =
  QModelIndex(h: fQSortFilterProxyModel_virtualbase_mapFromSource(self.h, sourceIndex.h))

type QSortFilterProxyModelmapFromSourceProc* = proc(sourceIndex: QModelIndex): QModelIndex
...
proc miqt_exec_callback_QSortFilterProxyModel_mapFromSource(self: ptr cQSortFilterProxyModel, slot: int, sourceIndex: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_mapFromSource ".} =
  ...
  let slotval1 = QModelIndex(h: sourceIndex)


  let virtualReturn = nimfunc[](slotval1 )

  virtualReturn.h
proc QSortFilterProxyModelmapSelectionToSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, proxySelection: QItemSelection): QItemSelection =
  QItemSelection(h: fQSortFilterProxyModel_virtualbase_mapSelectionToSource(self.h, proxySelection.h))

type QSortFilterProxyModelmapSelectionToSourceProc* = proc(proxySelection: QItemSelection): QItemSelection
...
proc miqt_exec_callback_QSortFilterProxyModel_mapSelectionToSource(self: ptr cQSortFilterProxyModel, slot: int, proxySelection: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_mapSelectionToSource ".} =
  ...
  let slotval1 = QItemSelection(h: proxySelection)


  let virtualReturn = nimfunc[](slotval1 )

  virtualReturn.h
proc QSortFilterProxyModelmapSelectionFromSource*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceSelection: QItemSelection): QItemSelection =
  QItemSelection(h: fQSortFilterProxyModel_virtualbase_mapSelectionFromSource(self.h, sourceSelection.h))

type QSortFilterProxyModelmapSelectionFromSourceProc* = proc(sourceSelection: QItemSelection): QItemSelection
...
proc miqt_exec_callback_QSortFilterProxyModel_mapSelectionFromSource(self: ptr cQSortFilterProxyModel, slot: int, sourceSelection: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_mapSelectionFromSource ".} =
  ...
  let slotval1 = QItemSelection(h: sourceSelection)


  let virtualReturn = nimfunc[](slotval1 )

  virtualReturn.h
proc QSortFilterProxyModelfilterAcceptsRow*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, source_row: cint, source_parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_filterAcceptsRow(self.h, source_row, source_parent.h)

type QSortFilterProxyModelfilterAcceptsRowProc* = proc(source_row: cint, source_parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_filterAcceptsRow(self: ptr cQSortFilterProxyModel, slot: int, source_row: cint, source_parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_filterAcceptsRow ".} =
  ...
  let slotval2 = QModelIndex(h: source_parent)


  let virtualReturn = nimfunc[](slotval1, slotval2 )

  virtualReturn
proc QSortFilterProxyModelfilterAcceptsColumn*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, source_column: cint, source_parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_filterAcceptsColumn(self.h, source_column, source_parent.h)

type QSortFilterProxyModelfilterAcceptsColumnProc* = proc(source_column: cint, source_parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_filterAcceptsColumn(self: ptr cQSortFilterProxyModel, slot: int, source_column: cint, source_parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_filterAcceptsColumn ".} =
  ...
  virtualReturn
proc QSortFilterProxyModellessThan*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, source_left: QModelIndex, source_right: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_lessThan(self.h, source_left.h, source_right.h)

type QSortFilterProxyModellessThanProc* = proc(source_left: QModelIndex, source_right: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_lessThan(self: ptr cQSortFilterProxyModel, slot: int, source_left: pointer, source_right: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_lessThan ".} =
  ...
  let slotval1 = QModelIndex(h: source_left)

  let slotval2 = QModelIndex(h: source_right)


  let virtualReturn = nimfunc[](slotval1, slotval2 )

  virtualReturn
proc QSortFilterProxyModelindex*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, column: cint, parent: QModelIndex): QModelIndex =
  QModelIndex(h: fQSortFilterProxyModel_virtualbase_index(self.h, row, column, parent.h))

type QSortFilterProxyModelindexProc* = proc(row: cint, column: cint, parent: QModelIndex): QModelIndex
...
proc miqt_exec_callback_QSortFilterProxyModel_index(self: ptr cQSortFilterProxyModel, slot: int, row: cint, column: cint, parent: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_index ".} =
  ...
  virtualReturn.h
proc QSortFilterProxyModelparent*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, child: QModelIndex): QModelIndex =
  QModelIndex(h: fQSortFilterProxyModel_virtualbase_parent(self.h, child.h))

type QSortFilterProxyModelparentProc* = proc(child: QModelIndex): QModelIndex
...
proc miqt_exec_callback_QSortFilterProxyModel_parent(self: ptr cQSortFilterProxyModel, slot: int, child: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_parent ".} =
  ...
  virtualReturn.h
proc QSortFilterProxyModelsibling*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, column: cint, idx: QModelIndex): QModelIndex =
  QModelIndex(h: fQSortFilterProxyModel_virtualbase_sibling(self.h, row, column, idx.h))

type QSortFilterProxyModelsiblingProc* = proc(row: cint, column: cint, idx: QModelIndex): QModelIndex
...
proc miqt_exec_callback_QSortFilterProxyModel_sibling(self: ptr cQSortFilterProxyModel, slot: int, row: cint, column: cint, idx: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_sibling ".} =
  ...
  virtualReturn.h
proc QSortFilterProxyModelrowCount*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): cint =
  fQSortFilterProxyModel_virtualbase_rowCount(self.h, parent.h)

type QSortFilterProxyModelrowCountProc* = proc(parent: QModelIndex): cint
...
proc miqt_exec_callback_QSortFilterProxyModel_rowCount(self: ptr cQSortFilterProxyModel, slot: int, parent: pointer): cint {.exportc: "miqt_exec_callback_QSortFilterProxyModel_rowCount ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelcolumnCount*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): cint =
  fQSortFilterProxyModel_virtualbase_columnCount(self.h, parent.h)

type QSortFilterProxyModelcolumnCountProc* = proc(parent: QModelIndex): cint
...
proc miqt_exec_callback_QSortFilterProxyModel_columnCount(self: ptr cQSortFilterProxyModel, slot: int, parent: pointer): cint {.exportc: "miqt_exec_callback_QSortFilterProxyModel_columnCount ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelhasChildren*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_hasChildren(self.h, parent.h)

type QSortFilterProxyModelhasChildrenProc* = proc(parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_hasChildren(self: ptr cQSortFilterProxyModel, slot: int, parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_hasChildren ".} =
  ...
  virtualReturn
proc QSortFilterProxyModeldata*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex, role: cint): QVariant =
  QVariant(h: fQSortFilterProxyModel_virtualbase_data(self.h, index.h, role))

type QSortFilterProxyModeldataProc* = proc(index: QModelIndex, role: cint): QVariant
...
proc miqt_exec_callback_QSortFilterProxyModel_data(self: ptr cQSortFilterProxyModel, slot: int, index: pointer, role: cint): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_data ".} =
  ...
  virtualReturn.h
proc QSortFilterProxyModelsetData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex, value: QVariant, role: cint): bool =
  fQSortFilterProxyModel_virtualbase_setData(self.h, index.h, value.h, role)

type QSortFilterProxyModelsetDataProc* = proc(index: QModelIndex, value: QVariant, role: cint): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_setData(self: ptr cQSortFilterProxyModel, slot: int, index: pointer, value: pointer, role: cint): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_setData ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelheaderData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, section: cint, orientation: cint, role: cint): QVariant =
  QVariant(h: fQSortFilterProxyModel_virtualbase_headerData(self.h, section, cint(orientation), role))

type QSortFilterProxyModelheaderDataProc* = proc(section: cint, orientation: cint, role: cint): QVariant
...
proc miqt_exec_callback_QSortFilterProxyModel_headerData(self: ptr cQSortFilterProxyModel, slot: int, section: cint, orientation: cint, role: cint): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_headerData ".} =
  ...
  virtualReturn.h
proc QSortFilterProxyModelsetHeaderData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, section: cint, orientation: cint, value: QVariant, role: cint): bool =
  fQSortFilterProxyModel_virtualbase_setHeaderData(self.h, section, cint(orientation), value.h, role)

type QSortFilterProxyModelsetHeaderDataProc* = proc(section: cint, orientation: cint, value: QVariant, role: cint): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_setHeaderData(self: ptr cQSortFilterProxyModel, slot: int, section: cint, orientation: cint, value: pointer, role: cint): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_setHeaderData ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelmimeData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, indexes: seq[QModelIndex]): QMimeData =
  var indexes_CArray = newSeq[pointer](len(indexes))
  for i in 0..<len(indexes):
    indexes_CArray[i] = indexes[i].h

  QMimeData(h: fQSortFilterProxyModel_virtualbase_mimeData(self.h, struct_miqt_array(len: csize_t(len(indexes)), data: if len(indexes) == 0: nil else: addr(indexes_CArray[0]))))

type QSortFilterProxyModelmimeDataProc* = proc(indexes: seq[QModelIndex]): QMimeData
...
proc miqt_exec_callback_QSortFilterProxyModel_mimeData(self: ptr cQSortFilterProxyModel, slot: int, indexes: struct_miqt_array): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_mimeData ".} =
  ...
  virtualReturn.h
proc QSortFilterProxyModeldropMimeData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, data: QMimeData, action: cint, row: cint, column: cint, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_dropMimeData(self.h, data.h, cint(action), row, column, parent.h)

type QSortFilterProxyModeldropMimeDataProc* = proc(data: QMimeData, action: cint, row: cint, column: cint, parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_dropMimeData(self: ptr cQSortFilterProxyModel, slot: int, data: pointer, action: cint, row: cint, column: cint, parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_dropMimeData ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelinsertRows*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, count: cint, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_insertRows(self.h, row, count, parent.h)

type QSortFilterProxyModelinsertRowsProc* = proc(row: cint, count: cint, parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_insertRows(self: ptr cQSortFilterProxyModel, slot: int, row: cint, count: cint, parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_insertRows ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelinsertColumns*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, column: cint, count: cint, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_insertColumns(self.h, column, count, parent.h)

type QSortFilterProxyModelinsertColumnsProc* = proc(column: cint, count: cint, parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_insertColumns(self: ptr cQSortFilterProxyModel, slot: int, column: cint, count: cint, parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_insertColumns ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelremoveRows*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, row: cint, count: cint, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_removeRows(self.h, row, count, parent.h)

type QSortFilterProxyModelremoveRowsProc* = proc(row: cint, count: cint, parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_removeRows(self: ptr cQSortFilterProxyModel, slot: int, row: cint, count: cint, parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_removeRows ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelremoveColumns*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, column: cint, count: cint, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_removeColumns(self.h, column, count, parent.h)

type QSortFilterProxyModelremoveColumnsProc* = proc(column: cint, count: cint, parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_removeColumns(self: ptr cQSortFilterProxyModel, slot: int, column: cint, count: cint, parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_removeColumns ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelfetchMore*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): void =
  fQSortFilterProxyModel_virtualbase_fetchMore(self.h, parent.h)

type QSortFilterProxyModelfetchMoreProc* = proc(parent: QModelIndex): void
...
proc miqt_exec_callback_QSortFilterProxyModel_fetchMore(self: ptr cQSortFilterProxyModel, slot: int, parent: pointer): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_fetchMore ".} =
  ...
  nimfunc[](slotval1)
proc QSortFilterProxyModelcanFetchMore*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_canFetchMore(self.h, parent.h)

type QSortFilterProxyModelcanFetchMoreProc* = proc(parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_canFetchMore(self: ptr cQSortFilterProxyModel, slot: int, parent: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_canFetchMore ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelflags*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): cint =
  cint(fQSortFilterProxyModel_virtualbase_flags(self.h, index.h))

type QSortFilterProxyModelflagsProc* = proc(index: QModelIndex): cint
...
proc miqt_exec_callback_QSortFilterProxyModel_flags(self: ptr cQSortFilterProxyModel, slot: int, index: pointer): cint {.exportc: "miqt_exec_callback_QSortFilterProxyModel_flags ".} =
  ...
  cint(virtualReturn)
proc QSortFilterProxyModelbuddy*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): QModelIndex =
  QModelIndex(h: fQSortFilterProxyModel_virtualbase_buddy(self.h, index.h))

type QSortFilterProxyModelbuddyProc* = proc(index: QModelIndex): QModelIndex
...
proc miqt_exec_callback_QSortFilterProxyModel_buddy(self: ptr cQSortFilterProxyModel, slot: int, index: pointer): pointer {.exportc: "miqt_exec_callback_QSortFilterProxyModel_buddy ".} =
  ...
  virtualReturn.h
proc QSortFilterProxyModelmatch*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, start: QModelIndex, role: cint, value: QVariant, hits: cint, flags: cint): seq[QModelIndex] =
...
type QSortFilterProxyModelmatchProc* = proc(start: QModelIndex, role: cint, value: QVariant, hits: cint, flags: cint): seq[QModelIndex]
...
proc QSortFilterProxyModelspan*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): QSize =
  QSize(h: fQSortFilterProxyModel_virtualbase_span(self.h, index.h))

type QSortFilterProxyModelspanProc* = proc(index: QModelIndex): QSize
...
proc miqt_exec_callback_QSortFilterProxyModel_revert(self: ptr cQSortFilterProxyModel, slot: int): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_revert ".} =
  ...
  nimfunc[]()
proc QSortFilterProxyModelitemData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): Table[cint,QVariant] =
...
type QSortFilterProxyModelitemDataProc* = proc(index: QModelIndex): Table[cint,QVariant]
...
proc QSortFilterProxyModelsetItemData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex, roles: Table[cint,QVariant]): bool =
...
type QSortFilterProxyModelsetItemDataProc* = proc(index: QModelIndex, roles: Table[cint,QVariant]): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_setItemData(self: ptr cQSortFilterProxyModel, slot: int, index: pointer, roles: struct_miqt_map): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_setItemData ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelclearItemData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_clearItemData(self.h, index.h)

type QSortFilterProxyModelclearItemDataProc* = proc(index: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_clearItemData(self: ptr cQSortFilterProxyModel, slot: int, index: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_clearItemData ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelcanDropMimeData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, data: QMimeData, action: cint, row: cint, column: cint, parent: QModelIndex): bool =
  fQSortFilterProxyModel_virtualbase_canDropMimeData(self.h, data.h, cint(action), row, column, parent.h)

type QSortFilterProxyModelcanDropMimeDataProc* = proc(data: QMimeData, action: cint, row: cint, column: cint, parent: QModelIndex): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_roleNames(self: ptr cQSortFilterProxyModel, slot: int): struct_miqt_map {.exportc: "miqt_exec_callback_QSortFilterProxyModel_roleNames ".} =
  ...
  struct_miqt_map(len: csize_t(len(virtualReturn)),keys: if len(virtualReturn) == 0: nil else: addr(virtualReturn_Keys_CArray[0]), values: if len(virtualReturn) == 0: nil else: addr(virtualReturn_Values_CArray[0]),)
proc QSortFilterProxyModelmoveRows*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceParent: QModelIndex, sourceRow: cint, count: cint, destinationParent: QModelIndex, destinationChild: cint): bool =
  fQSortFilterProxyModel_virtualbase_moveRows(self.h, sourceParent.h, sourceRow, count, destinationParent.h, destinationChild)

type QSortFilterProxyModelmoveRowsProc* = proc(sourceParent: QModelIndex, sourceRow: cint, count: cint, destinationParent: QModelIndex, destinationChild: cint): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_moveRows(self: ptr cQSortFilterProxyModel, slot: int, sourceParent: pointer, sourceRow: cint, count: cint, destinationParent: pointer, destinationChild: cint): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_moveRows ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelmoveColumns*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, sourceParent: QModelIndex, sourceColumn: cint, count: cint, destinationParent: QModelIndex, destinationChild: cint): bool =
...
type QSortFilterProxyModelmoveColumnsProc* = proc(sourceParent: QModelIndex, sourceColumn: cint, count: cint, destinationParent: QModelIndex, destinationChild: cint): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_moveColumns(self: ptr cQSortFilterProxyModel, slot: int, sourceParent: pointer, sourceColumn: cint, count: cint, destinationParent: pointer, destinationChild: cint): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_moveColumns ".} =
  ...
  virtualReturn
proc QSortFilterProxyModelmultiData*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, index: QModelIndex, roleDataSpan: QModelRoleDataSpan): void =
  fQSortFilterProxyModel_virtualbase_multiData(self.h, index.h, roleDataSpan.h)

type QSortFilterProxyModelmultiDataProc* = proc(index: QModelIndex, roleDataSpan: QModelRoleDataSpan): void
...
proc miqt_exec_callback_QSortFilterProxyModel_resetInternalData(self: ptr cQSortFilterProxyModel, slot: int): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_resetInternalData ".} =
  ...
  nimfunc[]()
proc QSortFilterProxyModelevent*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, event: QEvent): bool =
  fQSortFilterProxyModel_virtualbase_event(self.h, event.h)

type QSortFilterProxyModeleventProc* = proc(event: QEvent): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_event(self: ptr cQSortFilterProxyModel, slot: int, event: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_event ".} =
  ...
  virtualReturn
proc QSortFilterProxyModeleventFilter*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, watched: QObject, event: QEvent): bool =
  fQSortFilterProxyModel_virtualbase_eventFilter(self.h, watched.h, event.h)

type QSortFilterProxyModeleventFilterProc* = proc(watched: QObject, event: QEvent): bool
...
proc miqt_exec_callback_QSortFilterProxyModel_eventFilter(self: ptr cQSortFilterProxyModel, slot: int, watched: pointer, event: pointer): bool {.exportc: "miqt_exec_callback_QSortFilterProxyModel_eventFilter ".} =
  ...
  virtualReturn
proc QSortFilterProxyModeltimerEvent*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, event: QTimerEvent): void =
  fQSortFilterProxyModel_virtualbase_timerEvent(self.h, event.h)

type QSortFilterProxyModeltimerEventProc* = proc(event: QTimerEvent): void
...
proc miqt_exec_callback_QSortFilterProxyModel_timerEvent(self: ptr cQSortFilterProxyModel, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_timerEvent ".} =
  ...
  nimfunc[](slotval1)
proc QSortFilterProxyModelchildEvent*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, event: QChildEvent): void =
  fQSortFilterProxyModel_virtualbase_childEvent(self.h, event.h)

type QSortFilterProxyModelchildEventProc* = proc(event: QChildEvent): void
...
proc miqt_exec_callback_QSortFilterProxyModel_childEvent(self: ptr cQSortFilterProxyModel, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_childEvent ".} =
  ...
  nimfunc[](slotval1)
proc QSortFilterProxyModelcustomEvent*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, event: QEvent): void =
  fQSortFilterProxyModel_virtualbase_customEvent(self.h, event.h)

type QSortFilterProxyModelcustomEventProc* = proc(event: QEvent): void
...
proc miqt_exec_callback_QSortFilterProxyModel_customEvent(self: ptr cQSortFilterProxyModel, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_customEvent ".} =
  ...
  nimfunc[](slotval1)
proc QSortFilterProxyModelconnectNotify*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, signal: QMetaMethod): void =
  fQSortFilterProxyModel_virtualbase_connectNotify(self.h, signal.h)

type QSortFilterProxyModelconnectNotifyProc* = proc(signal: QMetaMethod): void
...
proc miqt_exec_callback_QSortFilterProxyModel_connectNotify(self: ptr cQSortFilterProxyModel, slot: int, signal: pointer): void {.exportc: "miqt_exec_callback_QSortFilterProxyModel_connectNotify ".} =
  ...
  nimfunc[](slotval1)
proc QSortFilterProxyModeldisconnectNotify*(self: gen_qsortfilterproxymodel_types.QSortFilterProxyModel, signal: QMetaMethod): void =
  fQSortFilterProxyModel_virtualbase_disconnectNotify(self.h, signal.h)

type QSortFilterProxyModeldisconnectNotifyProc* = proc(signal: QMetaMethod): void
