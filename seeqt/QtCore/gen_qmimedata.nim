...
proc QMimeDataretrieveData*(self: gen_qmimedata_types.QMimeData, mimetype: string, preferredType: QMetaType): QVariant =
  QVariant(h: fQMimeData_virtualbase_retrieveData(self.h, struct_miqt_string(data: mimetype, len: csize_t(len(mimetype))), preferredType.h))

type QMimeDataretrieveDataProc* = proc(mimetype: string, preferredType: QMetaType): QVariant
...
proc miqt_exec_callback_QMimeData_retrieveData(self: ptr cQMimeData, slot: int, mimetype: struct_miqt_string, preferredType: pointer): pointer {.exportc: "miqt_exec_callback_QMimeData_retrieveData ".} =
  ...
  let slotval2 = QMetaType(h: preferredType)
