...
proc styleString*(_: type gen_qfontdatabase_types.QFontDatabase, font: QFont): string =
  let v_ms = fcQFontDatabase_styleString(font.h)
  let vx_ret = string.fromBytes(toOpenArrayByte(v_ms.data, 0, int(v_ms.len)-1))
  c_free(v_ms.data)
  vx_ret

proc styleString*(_: type gen_qfontdatabase_types.QFontDatabase, fontInfo: QFontInfo): string =
  let v_ms = fcQFontDatabase_styleStringWithFontInfo(fontInfo.h)
  let vx_ret = string.fromBytes(toOpenArrayByte(v_ms.data, 0, int(v_ms.len)-1))
  c_free(v_ms.data)
  vx_ret

proc font*(_: type gen_qfontdatabase_types.QFontDatabase, family: string, style: string, pointSize: cint): QFont =
  QFont(h: fcQFontDatabase_font(struct_miqt_string(data: family, len: csize_t(len(family))), struct_miqt_string(data: style, len: csize_t(len(style))), pointSize))
