...
proc create*(T: type gen_qcursor_types.QCursor, bitmap: QBitmap, mask: QBitmap): gen_qcursor_types.QCursor =
  gen_qcursor_types.QCursor.init(fcQCursor_new3(bitmap.h, mask.h))

proc create*(T: type gen_qcursor_types.QCursor, pixmap: QPixmap): gen_qcursor_types.QCursor =
  gen_qcursor_types.QCursor.init(fcQCursor_new4(pixmap.h))

proc create*(T: type gen_qcursor_types.QCursor, cursor: QCursor): gen_qcursor_types.QCursor =
  gen_qcursor_types.QCursor.init(fcQCursor_new5(cursor.h))

proc create*(T: type gen_qcursor_types.QCursor, bitmap: QBitmap, mask: QBitmap, hotX: cint): gen_qcursor_types.QCursor =
  gen_qcursor_types.QCursor.init(fcQCursor_new6(bitmap.h, mask.h, hotX))

proc create*(T: type gen_qcursor_types.QCursor, bitmap: QBitmap, mask: QBitmap, hotX: cint, hotY: cint): gen_qcursor_types.QCursor =
  gen_qcursor_types.QCursor.init(fcQCursor_new7(bitmap.h, mask.h, hotX, hotY))

proc create*(T: type gen_qcursor_types.QCursor, pixmap: QPixmap, hotX: cint): gen_qcursor_types.QCursor =
  gen_qcursor_types.QCursor.init(fcQCursor_new8(pixmap.h, hotX))

proc create*(T: type gen_qcursor_types.QCursor, pixmap: QPixmap, hotX: cint, hotY: cint): gen_qcursor_types.QCursor =
  gen_qcursor_types.QCursor.init(fcQCursor_new9(pixmap.h, hotX, hotY))

proc operatorAssign*(self: gen_qcursor_types.QCursor, cursor: QCursor): void =
  fcQCursor_operatorAssign(self.h, cursor.h)

proc swap*(self: gen_qcursor_types.QCursor, other: QCursor): void =
  fcQCursor_swap(self.h, other.h)

proc ToQVariant*(self: gen_qcursor_types.QCursor, ): QVariant =
  QVariant(h: fcQCursor_ToQVariant(self.h))
...
proc bitmap*(self: gen_qcursor_types.QCursor, ): QBitmap =
  QBitmap(h: fcQCursor_bitmap(self.h))

proc mask*(self: gen_qcursor_types.QCursor, ): QBitmap =
  QBitmap(h: fcQCursor_mask(self.h))

proc bitmap*(self: gen_qcursor_types.QCursor, param1: cint): QBitmap =
  QBitmap(h: fcQCursor_bitmapWithQtReturnByValueConstant(self.h, cint(param1)))

proc mask*(self: gen_qcursor_types.QCursor, param1: cint): QBitmap =
  QBitmap(h: fcQCursor_maskWithQtReturnByValueConstant(self.h, cint(param1)))

proc pixmap*(self: gen_qcursor_types.QCursor, ): QPixmap =
  QPixmap(h: fcQCursor_pixmap(self.h))

proc hotSpot*(self: gen_qcursor_types.QCursor, ): QPoint =
  QPoint(h: fcQCursor_hotSpot(self.h))

proc pos*(_: type gen_qcursor_types.QCursor, ): QPoint =
  QPoint(h: fcQCursor_pos())

proc pos*(_: type gen_qcursor_types.QCursor, screen: QScreen): QPoint =
  QPoint(h: fcQCursor_posWithScreen(screen.h))

proc setPos*(_: type gen_qcursor_types.QCursor, x: cint, y: cint): void =
  fcQCursor_setPos(x, y)

proc setPos*(_: type gen_qcursor_types.QCursor, screen: QScreen, x: cint, y: cint): void =
  fcQCursor_setPos2(screen.h, x, y)

proc setPos*(_: type gen_qcursor_types.QCursor, p: QPoint): void =
  fcQCursor_setPosWithQPoint(p.h)

proc setPos*(_: type gen_qcursor_types.QCursor, screen: QScreen, p: QPoint): void =
