...
proc miqt_exec_callback_QIconEngine_isNull(self: ptr cQIconEngine, slot: int): bool {.exportc: "miqt_exec_callback_QIconEngine_isNull ".} =
  ...
  virtualReturn
proc QIconEnginescaledPixmap*(self: gen_qiconengine_types.QIconEngine, size: QSize, mode: cint, state: cint, scale: float64): QPixmap =
  QPixmap(h: fQIconEngine_virtualbase_scaledPixmap(self.h, size.h, cint(mode), cint(state), scale))

type QIconEnginescaledPixmapProc* = proc(size: QSize, mode: cint, state: cint, scale: float64): QPixmap
