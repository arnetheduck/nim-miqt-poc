...
QQuickRenderTarget* QQuickRenderTarget_fromVulkanImage(VkImage image, VkImageLayout layout, VkFormat format, QSize* pixelSize) {
	return new QQuickRenderTarget(QQuickRenderTarget::fromVulkanImage(image, layout, format, *pixelSize));
}

QQuickRenderTarget* QQuickRenderTarget_fromVulkanImage2(VkImage image, VkImageLayout layout, QSize* pixelSize) {
	return new QQuickRenderTarget(QQuickRenderTarget::fromVulkanImage(image, layout, *pixelSize));
}
...
QQuickRenderTarget* QQuickRenderTarget_fromVulkanImage5(VkImage image, VkImageLayout layout, VkFormat format, QSize* pixelSize, int sampleCount) {
	return new QQuickRenderTarget(QQuickRenderTarget::fromVulkanImage(image, layout, format, *pixelSize, static_cast<int>(sampleCount)));
}

QQuickRenderTarget* QQuickRenderTarget_fromVulkanImage4(VkImage image, VkImageLayout layout, QSize* pixelSize, int sampleCount) {
	return new QQuickRenderTarget(QQuickRenderTarget::fromVulkanImage(image, layout, *pixelSize, static_cast<int>(sampleCount)));
}
