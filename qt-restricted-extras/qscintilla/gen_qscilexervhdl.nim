...
proc trUtf8*(_: type gen_qscilexervhdl_types.QsciLexerVHDL, s: cstring, c: cstring): string =
  let v_ms = fcQsciLexerVHDL_trUtf82(s, c)
  let vx_ret = string.fromBytes(toOpenArrayByte(v_ms.data, 0, int(v_ms.len)-1))
  c_free(v_ms.data)
  vx_ret

proc trUtf8*(_: type gen_qscilexervhdl_types.QsciLexerVHDL, s: cstring, c: cstring, n: cint): string =
