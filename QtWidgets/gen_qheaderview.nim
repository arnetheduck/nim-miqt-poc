...
proc miqt_exec_callback_cQHeaderView_sortIndicatorClearableChanged(slot: int, clearable: bool) {.exportc: "miqt_exec_callback_QHeaderView_sortIndicatorClearableChanged".} =
...
proc onsortIndicatorClearableChanged*(self: gen_qheaderview_types.QHeaderView, slot: QHeaderViewsortIndicatorClearableChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQHeaderView_connect_sortIndicatorClearableChanged(self.h, cast[int](addr tmp[]))
...
type QHeaderVieweditProc* = proc(self: QHeaderView, index: gen_qabstractitemmodel_types.QModelIndex, trigger: cint, event: gen_qcoreevent_types.QEvent): bool {.raises: [], gcsafe.}
...
proc miqt_exec_callback_cQHeaderView_sectionSizeFromContents(vtbl: pointer, self: pointer, logicalIndex: cint): pointer {.cdecl.} =
  ...
  let self = QHeaderView(h: self)
  let slotval1 = logicalIndex
  let virtualReturn = vtbl[].sectionSizeFromContents(self, slotval1)
  virtualReturn.h
...
proc miqt_exec_callback_cQHeaderView_itemDelegateForIndex(vtbl: pointer, self: pointer, index: pointer): pointer {.cdecl.} =
  ...
  let self = QHeaderView(h: self)
  let slotval1 = gen_qabstractitemmodel_types.QModelIndex(h: index)
  let virtualReturn = vtbl[].itemDelegateForIndex(self, slotval1)
  virtualReturn.h
...
proc create*(T: type gen_qheaderview_types.QHeaderView,
  ...
  if not isNil(vtbl.edit):
    vtbl[].vtbl.edit = miqt_exec_callback_cQHeaderView_edit2
