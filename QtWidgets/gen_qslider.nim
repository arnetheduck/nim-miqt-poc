...
proc fcQSlider_virtualbase_nativeEvent(self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QSlider_virtualbase_nativeEvent".}
...
type QSlidernativeEventProc* = proc(self: QSlider, eventType: seq[byte], message: pointer, resultVal: ptr clong): bool {.raises: [], gcsafe.}
...
proc miqt_exec_callback_cQSlider_nativeEvent(vtbl: pointer, self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QSliderVTable](vtbl)
  let self = QSlider(h: self)
