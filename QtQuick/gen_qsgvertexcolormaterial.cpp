...
public:
	...
	QSGMaterialShader* virtualbase_createShader(int renderMode) const {
		...
		return QSGVertexColorMaterial::createShader(static_cast<QSGRendererInterface::RenderMode>(renderMode));
...
QSGMaterialShader* QSGVertexColorMaterial_virtualbase_createShader(const void* self, int renderMode) {
	return ( (const MiqtVirtualQSGVertexColorMaterial*)(self) )->virtualbase_createShader(renderMode);
}
