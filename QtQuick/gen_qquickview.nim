...
proc miqt_exec_callback_cQQuickView_closeEvent(vtbl: pointer, self: pointer, param1: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QQuickViewVTable](vtbl)
  let self = QQuickView(h: self)
...
proc miqt_exec_callback_cQQuickView_paintEvent(vtbl: pointer, self: pointer, param1: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QQuickViewVTable](vtbl)
  let self = QQuickView(h: self)
...
proc miqt_exec_callback_cQQuickView_nativeEvent(vtbl: pointer, self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QQuickViewVTable](vtbl)
  let self = QQuickView(h: self)
...
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  if not isNil(vtbl.closeEvent):
    vtbl[].vtbl.closeEvent = miqt_exec_callback_cQQuickView_closeEvent
  ...
  if not isNil(vtbl.paintEvent):
    vtbl[].vtbl.paintEvent = miqt_exec_callback_cQQuickView_paintEvent
