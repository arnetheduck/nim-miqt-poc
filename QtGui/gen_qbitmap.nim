...
proc fcQBitmap_new6(vtbl: pointer, param1: pointer): ptr cQBitmap {.importc: "QBitmap_new6".}
...
proc fromPixmap*(_: type gen_qbitmap_types.QBitmap, pixmap: gen_qpixmap_types.QPixmap): gen_qbitmap_types.QBitmap =
...
proc create*(T: type gen_qbitmap_types.QBitmap,
    param1: gen_qbitmap_types.QBitmap,
    vtbl: ref QBitmapVTable = nil): gen_qbitmap_types.QBitmap =
  let vtbl = if vtbl == nil: new QBitmapVTable else: vtbl
  GC_ref(vtbl)
  vtbl.vtbl.destructor = proc(vtbl: ptr cQBitmapVTable, _: ptr cQBitmap) {.cdecl.} =
  ...
  if not isNil(vtbl.sharedPainter):
    vtbl[].vtbl.sharedPainter = miqt_exec_callback_cQBitmap_sharedPainter
  gen_qbitmap_types.QBitmap(h: fcQBitmap_new6(addr(vtbl[]), param1.h))
