...
proc miqt_exec_callback_cQRubberBand_initStyleOption(vtbl: pointer, self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QRubberBandVTable](vtbl)
  let self = QRubberBand(h: self)
...
proc miqt_exec_callback_cQRubberBand_nativeEvent(vtbl: pointer, self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QRubberBandVTable](vtbl)
  let self = QRubberBand(h: self)
...
proc create*(T: type gen_qrubberband_types.QRubberBand,
  ...
  if not isNil(vtbl.initStyleOption):
    vtbl[].vtbl.initStyleOption = miqt_exec_callback_cQRubberBand_initStyleOption
