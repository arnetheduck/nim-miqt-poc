...
proc miqt_exec_callback_cQGraphicsProxyWidget_isEmpty(vtbl: pointer, self: pointer): bool {.cdecl.} =
  let vtbl = cast[ptr QGraphicsProxyWidgetVTable](vtbl)
  let self = QGraphicsProxyWidget(h: self)
...
proc create*(T: type gen_qgraphicsproxywidget_types.QGraphicsProxyWidget,
  ...
  if not isNil(vtbl.isEmpty):
    vtbl[].vtbl.isEmpty = miqt_exec_callback_cQGraphicsProxyWidget_isEmpty
