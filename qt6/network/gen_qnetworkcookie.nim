...
type QNetworkCookieRawForm* = cint
const
  QNetworkCookieNameAndValueOnly* = 0
  QNetworkCookieFull* = 1



type QNetworkCookieSameSite* = cint
const
  QNetworkCookieDefault* = 0
  QNetworkCookieNone* = 1
  QNetworkCookieLax* = 2
  QNetworkCookieStrict* = 3
...
func init*(T: type QNetworkCookie, h: ptr cQNetworkCookie): QNetworkCookie =
  T(h: h)
proc create*(T: type QNetworkCookie, ): QNetworkCookie =

  QNetworkCookie.init(fcQNetworkCookie_new())
proc create*(T: type QNetworkCookie, other: QNetworkCookie): QNetworkCookie =

  QNetworkCookie.init(fcQNetworkCookie_new2(other.h))
proc create*(T: type QNetworkCookie, name: seq[byte]): QNetworkCookie =

  QNetworkCookie.init(fcQNetworkCookie_new3(struct_miqt_string(data: cast[cstring](if len(name) == 0: nil else: unsafeAddr name[0]), len: csize_t(len(name)))))
proc create*(T: type QNetworkCookie, name: seq[byte], value: seq[byte]): QNetworkCookie =

  QNetworkCookie.init(fcQNetworkCookie_new4(struct_miqt_string(data: cast[cstring](if len(name) == 0: nil else: unsafeAddr name[0]), len: csize_t(len(name))), struct_miqt_string(data: cast[cstring](if len(value) == 0: nil else: unsafeAddr value[0]), len: csize_t(len(value)))))
proc operatorAssign*(self: QNetworkCookie, other: QNetworkCookie): void =

  fcQNetworkCookie_operatorAssign(self.h, other.h)

proc swap*(self: QNetworkCookie, other: QNetworkCookie): void =

  fcQNetworkCookie_swap(self.h, other.h)

proc operatorEqual*(self: QNetworkCookie, other: QNetworkCookie): bool =

  fcQNetworkCookie_operatorEqual(self.h, other.h)

proc operatorNotEqual*(self: QNetworkCookie, other: QNetworkCookie): bool =

  fcQNetworkCookie_operatorNotEqual(self.h, other.h)

proc isSecure*(self: QNetworkCookie, ): bool =

  fcQNetworkCookie_isSecure(self.h)

proc setSecure*(self: QNetworkCookie, enable: bool): void =

  fcQNetworkCookie_setSecure(self.h, enable)

proc isHttpOnly*(self: QNetworkCookie, ): bool =

  fcQNetworkCookie_isHttpOnly(self.h)

proc setHttpOnly*(self: QNetworkCookie, enable: bool): void =

  fcQNetworkCookie_setHttpOnly(self.h, enable)

proc sameSitePolicy*(self: QNetworkCookie, ): QNetworkCookieSameSite =

  QNetworkCookieSameSite(fcQNetworkCookie_sameSitePolicy(self.h))

proc setSameSitePolicy*(self: QNetworkCookie, sameSite: QNetworkCookieSameSite): void =

  fcQNetworkCookie_setSameSitePolicy(self.h, cint(sameSite))

proc isSessionCookie*(self: QNetworkCookie, ): bool =

  fcQNetworkCookie_isSessionCookie(self.h)

proc expirationDate*(self: QNetworkCookie, ): gen_qdatetime.QDateTime =

  gen_qdatetime.QDateTime(h: fcQNetworkCookie_expirationDate(self.h))

proc setExpirationDate*(self: QNetworkCookie, date: gen_qdatetime.QDateTime): void =

  fcQNetworkCookie_setExpirationDate(self.h, date.h)

proc domain*(self: QNetworkCookie, ): string =
...
proc setDomain*(self: QNetworkCookie, domain: string): void =

  fcQNetworkCookie_setDomain(self.h, struct_miqt_string(data: domain, len: csize_t(len(domain))))

proc path*(self: QNetworkCookie, ): string =
...
proc setPath*(self: QNetworkCookie, path: string): void =

  fcQNetworkCookie_setPath(self.h, struct_miqt_string(data: path, len: csize_t(len(path))))

proc name*(self: QNetworkCookie, ): seq[byte] =
...
proc setName*(self: QNetworkCookie, cookieName: seq[byte]): void =
...
proc value*(self: QNetworkCookie, ): seq[byte] =
...
proc setValue*(self: QNetworkCookie, value: seq[byte]): void =
...
proc toRawForm*(self: QNetworkCookie, ): seq[byte] =
...
proc hasSameIdentifier*(self: QNetworkCookie, other: QNetworkCookie): bool =

  fcQNetworkCookie_hasSameIdentifier(self.h, other.h)

proc normalize*(self: QNetworkCookie, url: gen_qurl.QUrl): void =

  fcQNetworkCookie_normalize(self.h, url.h)

proc parseCookies*(_: type QNetworkCookie, cookieString: seq[byte]): seq[QNetworkCookie] =
...
proc toRawForm1*(self: QNetworkCookie, form: QNetworkCookieRawForm): seq[byte] =
