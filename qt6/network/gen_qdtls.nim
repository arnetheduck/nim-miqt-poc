...
type QDtlspskRequiredSlot* = proc(authenticator: gen_qsslpresharedkeyauthenticator.QSslPreSharedKeyAuthenticator)
...
proc setPeer*(self: gen_qdtls_types.QDtls, address: gen_qhostaddress.QHostAddress, port: cushort, verificationName: string): bool =
  fcQDtls_setPeer3(self.h, address.h, port, struct_miqt_string(data: verificationName, len: csize_t(len(verificationName))))

proc doHandshake*(self: gen_qdtls_types.QDtls, socket: gen_qudpsocket.QUdpSocket, dgram: seq[byte]): bool =
