...
proc socketStartedConnecting*(self: QNetworkReply, ): void =
...
proc onsocketStartedConnecting*(self: QNetworkReply, slot: proc()) =
  type Cb = proc()
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
  fQNetworkReply_connect_socketStartedConnecting(self.h, cast[int](addr tmp[]))
proc requestSent*(self: QNetworkReply, ): void =
...
proc onrequestSent*(self: QNetworkReply, slot: proc()) =
  type Cb = proc()
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
...
proc staticMetaObject*(_: type QNetworkReply): gen_qobjectdefs.QMetaObject =
