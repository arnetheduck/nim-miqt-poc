...
proc fcQSoundEffect_connect_audioDeviceChanged(self: pointer, slot: int) {.importc: "QSoundEffect_connect_audioDeviceChanged".}
...
proc onaudioDeviceChanged*(self: gen_qsoundeffect_types.QSoundEffect, slot: QSoundEffectaudioDeviceChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQSoundEffect_connect_audioDeviceChanged(self.h, cast[int](addr tmp[]))
