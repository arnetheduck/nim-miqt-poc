...
proc create*(T: type QSoundEffect, audioDevice: gen_qaudiodevice.QAudioDevice): QSoundEffect =
...
proc create*(T: type QSoundEffect, audioDevice: gen_qaudiodevice.QAudioDevice, parent: gen_qobject.QObject): QSoundEffect =
...
proc audioDevice*(self: QSoundEffect, ): gen_qaudiodevice.QAudioDevice =

  gen_qaudiodevice.QAudioDevice(h: fcQSoundEffect_audioDevice(self.h))

proc setAudioDevice*(self: QSoundEffect, device: gen_qaudiodevice.QAudioDevice): void =

  fcQSoundEffect_setAudioDevice(self.h, device.h)

proc volume*(self: QSoundEffect, ): float32 =

  fcQSoundEffect_volume(self.h)

proc setVolume*(self: QSoundEffect, volume: float32): void =
...
proc audioDeviceChanged*(self: QSoundEffect, ): void =
...
proc onaudioDeviceChanged*(self: QSoundEffect, slot: proc()) =
  type Cb = proc()
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
