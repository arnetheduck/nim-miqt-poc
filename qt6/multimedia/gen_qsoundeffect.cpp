...
void miqt_exec_callback_QSoundEffect_audioDeviceChanged_release(intptr_t);
#ifdef __cplusplus
} /* extern C */
#endif
...
void QSoundEffect_connect_audioDeviceChanged(QSoundEffect* self, intptr_t slot) {
	struct caller {
		...
		caller(const caller &) = delete;
		caller &operator=(const caller &) = delete;
		~caller() { miqt_exec_callback_QSoundEffect_audioDeviceChanged_release(slot); }
	};
	MiqtVirtualQSoundEffect::connect(self, static_cast<void (QSoundEffect::*)()>(&QSoundEffect::audioDeviceChanged), self, caller{slot});
}
...
QObject* QSoundEffect_protectedbase_sender(const void* self) {
	return ( (const MiqtVirtualQSoundEffect*)(self) )->protectedbase_sender();
}

int QSoundEffect_protectedbase_senderSignalIndex(const void* self) {
	return ( (const MiqtVirtualQSoundEffect*)(self) )->protectedbase_senderSignalIndex();
}

int QSoundEffect_protectedbase_receivers(const void* self, const char* signal) {
	return ( (const MiqtVirtualQSoundEffect*)(self) )->protectedbase_receivers(signal);
}

bool QSoundEffect_protectedbase_isSignalConnected(const void* self, QMetaMethod* signal) {
	return ( (const MiqtVirtualQSoundEffect*)(self) )->protectedbase_isSignalConnected(signal);
}
