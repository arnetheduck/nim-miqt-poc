...
proc videoSink*(self: QVideoWidget, ): gen_qvideosink.QVideoSink =

  gen_qvideosink.QVideoSink(h: fcQVideoWidget_videoSink(self.h))
...
proc aspectRatioModeChanged*(self: QVideoWidget, mode: gen_qnamespace.AspectRatioMode): void =
...
proc miqt_exec_callback_QVideoWidget_aspectRatioModeChanged(slot: int, mode: cint) {.exportc.} =
  type Cb = proc(mode: gen_qnamespace.AspectRatioMode)
  let nimfunc = cast[ptr Cb](cast[pointer](slot))
  let slotval1 = gen_qnamespace.AspectRatioMode(mode)


  nimfunc[](slotval1)

proc onaspectRatioModeChanged*(self: QVideoWidget, slot: proc(mode: gen_qnamespace.AspectRatioMode)) =
  type Cb = proc(mode: gen_qnamespace.AspectRatioMode)
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
...
proc miqt_exec_callback_QVideoWidget_focusOutEvent(self: ptr cQVideoWidget, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QVideoWidget_focusOutEvent ".} =
  ...
  nimfunc[](superCall, slotval1)
proc callVirtualBase_enterEvent(self: QVideoWidget, event: gen_qevent.QEnterEvent): void =


  fQVideoWidget_virtualbase_enterEvent(self.h, event.h)

type QVideoWidgetenterEventBase* = proc(event: gen_qevent.QEnterEvent): void
proc onenterEvent*(self: QVideoWidget, slot: proc(super: QVideoWidgetenterEventBase, event: gen_qevent.QEnterEvent): void) =
  # TODO check subclass
  type Cb = proc(super: QVideoWidgetenterEventBase, event: gen_qevent.QEnterEvent): void
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
...
proc miqt_exec_callback_QVideoWidget_dropEvent(self: ptr cQVideoWidget, slot: int, event: pointer): void {.exportc: "miqt_exec_callback_QVideoWidget_dropEvent ".} =
  ...
  nimfunc[](superCall, slotval1)
proc callVirtualBase_nativeEvent(self: QVideoWidget, eventType: seq[byte], message: pointer, resultVal: ptr uint): bool =
...
type QVideoWidgetnativeEventBase* = proc(eventType: seq[byte], message: pointer, resultVal: ptr uint): bool
proc onnativeEvent*(self: QVideoWidget, slot: proc(super: QVideoWidgetnativeEventBase, eventType: seq[byte], message: pointer, resultVal: ptr uint): bool) =
  # TODO check subclass
  type Cb = proc(super: QVideoWidgetnativeEventBase, eventType: seq[byte], message: pointer, resultVal: ptr uint): bool
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
