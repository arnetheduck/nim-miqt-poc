...
proc miqt_exec_callback_cQFrame_initStyleOption(vtbl: pointer, self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QFrameVTable](vtbl)
  let self = QFrame(h: self)
...
proc miqt_exec_callback_cQFrame_nativeEvent(vtbl: pointer, self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QFrameVTable](vtbl)
  let self = QFrame(h: self)
...
proc create*(T: type gen_qframe_types.QFrame,
  ...
  if not isNil(vtbl.initStyleOption):
    vtbl[].vtbl.initStyleOption = miqt_exec_callback_cQFrame_initStyleOption
