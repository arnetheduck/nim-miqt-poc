...
proc miqt_exec_callback_QIconEngine_iconName(self: ptr cQIconEngine, slot: int): struct_miqt_string {.exportc: "miqt_exec_callback_QIconEngine_iconName ".} =
  ...
  struct_miqt_string(data: virtualReturn, len: csize_t(len(virtualReturn)))
proc callVirtualBase_isNull(self: QIconEngine, ): bool =


  fQIconEngine_virtualbase_isNull(self.h)

type QIconEngineisNullBase* = proc(): bool
proc onisNull*(self: QIconEngine, slot: proc(super: QIconEngineisNullBase): bool) =
  # TODO check subclass
  type Cb = proc(super: QIconEngineisNullBase): bool
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
...
proc miqt_exec_callback_QIconEngine_isNull(self: ptr cQIconEngine, slot: int): bool {.exportc: "miqt_exec_callback_QIconEngine_isNull ".} =
  ...
  var nimfunc = cast[ptr Cb](cast[pointer](slot))
  proc superCall(): auto =
    callVirtualBase_isNull(QIconEngine(h: self), )

  let virtualReturn = nimfunc[](superCall )

  virtualReturn
proc callVirtualBase_scaledPixmap(self: QIconEngine, size: gen_qsize.QSize, mode: gen_qicon.QIconMode, state: gen_qicon.QIconState, scale: float64): gen_qpixmap.QPixmap =


  gen_qpixmap.QPixmap(h: fQIconEngine_virtualbase_scaledPixmap(self.h, size.h, cint(mode), cint(state), scale))

type QIconEnginescaledPixmapBase* = proc(size: gen_qsize.QSize, mode: gen_qicon.QIconMode, state: gen_qicon.QIconState, scale: float64): gen_qpixmap.QPixmap
proc onscaledPixmap*(self: QIconEngine, slot: proc(super: QIconEnginescaledPixmapBase, size: gen_qsize.QSize, mode: gen_qicon.QIconMode, state: gen_qicon.QIconState, scale: float64): gen_qpixmap.QPixmap) =
  # TODO check subclass
  type Cb = proc(super: QIconEnginescaledPixmapBase, size: gen_qsize.QSize, mode: gen_qicon.QIconMode, state: gen_qicon.QIconState, scale: float64): gen_qpixmap.QPixmap
  var tmp = new Cb
  tmp[] = slot
  GC_ref(tmp)
...
proc miqt_exec_callback_QIconEngine_scaledPixmap(self: ptr cQIconEngine, slot: int, size: pointer, mode: cint, state: cint, scale: float64): pointer {.exportc: "miqt_exec_callback_QIconEngine_scaledPixmap ".} =
  type Cb = proc(super: QIconEnginescaledPixmapBase, size: gen_qsize.QSize, mode: gen_qicon.QIconMode, state: gen_qicon.QIconState, scale: float64): gen_qpixmap.QPixmap
  var nimfunc = cast[ptr Cb](cast[pointer](slot))
  proc superCall(size: gen_qsize.QSize, mode: gen_qicon.QIconMode, state: gen_qicon.QIconState, scale: float64): auto =
    callVirtualBase_scaledPixmap(QIconEngine(h: self), size, mode, state, scale)
  let slotval1 = gen_qsize.QSize(h: size)
