...
type cQIconEngineVTable = object
  ...
  isNull*: proc(vtbl, self: pointer, ): bool {.cdecl, raises: [], gcsafe.}
  scaledPixmap*: proc(vtbl, self: pointer, size: pointer, mode: cint, state: cint, scale: float64): pointer {.cdecl, raises: [], gcsafe.}
...
proc miqt_exec_callback_cQIconEngine_actualSize(vtbl: pointer, self: pointer, size: pointer, mode: cint, state: cint): pointer {.cdecl.} =
  ...
  let virtualReturn = vtbl[].actualSize(self, slotval1, slotval2, slotval3)
  virtualReturn.h
...
proc miqt_exec_callback_cQIconEngine_pixmap(vtbl: pointer, self: pointer, size: pointer, mode: cint, state: cint): pointer {.cdecl.} =
  ...
  let slotval2 = cint(mode)
  let slotval3 = cint(state)
  let virtualReturn = vtbl[].pixmap(self, slotval1, slotval2, slotval3)
  virtualReturn.h
...
proc miqt_exec_callback_cQIconEngine_key(vtbl: pointer, self: pointer): struct_miqt_string {.cdecl.} =
  let vtbl = cast[ptr QIconEngineVTable](vtbl)
  let self = QIconEngine(h: self)
  let virtualReturn = vtbl[].key(self)
...
proc miqt_exec_callback_cQIconEngine_read(vtbl: pointer, self: pointer, inVal: pointer): bool {.cdecl.} =
  ...
  let virtualReturn = vtbl[].read(self, slotval1)
  virtualReturn
...
proc miqt_exec_callback_cQIconEngine_write(vtbl: pointer, self: pointer, outVal: pointer): bool {.cdecl.} =
  ...
  let virtualReturn = vtbl[].write(self, slotval1)
  virtualReturn
...
proc miqt_exec_callback_cQIconEngine_availableSizes(vtbl: pointer, self: pointer, mode: cint, state: cint): struct_miqt_array {.cdecl.} =
  ...
  let virtualReturn = vtbl[].availableSizes(self, slotval1, slotval2)
...
proc miqt_exec_callback_cQIconEngine_iconName(vtbl: pointer, self: pointer): struct_miqt_string {.cdecl.} =
  let vtbl = cast[ptr QIconEngineVTable](vtbl)
  let self = QIconEngine(h: self)
  let virtualReturn = vtbl[].iconName(self)
  struct_miqt_string(data: virtualReturn, len: csize_t(len(virtualReturn)))

proc QIconEngineisNull*(self: gen_qiconengine_types.QIconEngine, ): bool =
  fcQIconEngine_virtualbase_isNull(self.h)

proc miqt_exec_callback_cQIconEngine_isNull(vtbl: pointer, self: pointer): bool {.cdecl.} =
  let vtbl = cast[ptr QIconEngineVTable](vtbl)
  let self = QIconEngine(h: self)
  let virtualReturn = vtbl[].isNull(self)
  virtualReturn
...
proc QIconEnginescaledPixmap*(self: gen_qiconengine_types.QIconEngine, size: gen_qsize_types.QSize, mode: cint, state: cint, scale: float64): gen_qpixmap_types.QPixmap =
  gen_qpixmap_types.QPixmap(h: fcQIconEngine_virtualbase_scaledPixmap(self.h, size.h, cint(mode), cint(state), scale))

proc miqt_exec_callback_cQIconEngine_scaledPixmap(vtbl: pointer, self: pointer, size: pointer, mode: cint, state: cint, scale: float64): pointer {.cdecl.} =
  let vtbl = cast[ptr QIconEngineVTable](vtbl)
  let self = QIconEngine(h: self)
  let slotval1 = gen_qsize_types.QSize(h: size)
  let slotval2 = cint(mode)
  let slotval3 = cint(state)
  let slotval4 = scale
  let virtualReturn = vtbl[].scaledPixmap(self, slotval1, slotval2, slotval3, slotval4)
  virtualReturn.h
...
proc create*(T: type gen_qiconengine_types.QIconEngine,
  ...
  if not isNil(vtbl.isNull):
    vtbl[].vtbl.isNull = miqt_exec_callback_cQIconEngine_isNull
  if not isNil(vtbl.scaledPixmap):
    vtbl[].vtbl.scaledPixmap = miqt_exec_callback_cQIconEngine_scaledPixmap
