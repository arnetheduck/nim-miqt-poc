...
proc miqt_exec_callback_cQSplitter_createHandle(vtbl: pointer, self: pointer): pointer {.cdecl.} =
  let vtbl = cast[ptr QSplitterVTable](vtbl)
  let self = QSplitter(h: self)
  let virtualReturn = vtbl[].createHandle(self)
  virtualReturn.h
...
proc miqt_exec_callback_cQSplitter_initStyleOption(vtbl: pointer, self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QSplitterVTable](vtbl)
  let self = QSplitter(h: self)
...
proc miqt_exec_callback_cQSplitter_nativeEvent(vtbl: pointer, self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QSplitterVTable](vtbl)
  let self = QSplitter(h: self)
...
proc create*(T: type gen_qsplitter_types.QSplitter,
  ...
  if not isNil(vtbl.initStyleOption):
    vtbl[].vtbl.initStyleOption = miqt_exec_callback_cQSplitter_initStyleOption
...
proc miqt_exec_callback_cQSplitterHandle_nativeEvent(vtbl: pointer, self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QSplitterHandleVTable](vtbl)
  let self = QSplitterHandle(h: self)
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
