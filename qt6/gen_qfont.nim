...
proc create*(T: type QFont, families: seq[string]): QFont =
  ...
  QFont.init(fcQFont_new3(struct_miqt_array(len: csize_t(len(families)), data: if len(families) == 0: nil else: addr(families_CArray[0]))))
...
proc create*(T: type QFont, families: seq[string], pointSize: cint): QFont =

  var families_CArray = newSeq[struct_miqt_string](len(families))
  for i in 0..<len(families):
    families_CArray[i] = struct_miqt_string(data: families[i], len: csize_t(len(families[i])))

  QFont.init(fcQFont_new9(struct_miqt_array(len: csize_t(len(families)), data: if len(families) == 0: nil else: addr(families_CArray[0])), pointSize))
proc create*(T: type QFont, families: seq[string], pointSize: cint, weight: cint): QFont =

  var families_CArray = newSeq[struct_miqt_string](len(families))
  for i in 0..<len(families):
    families_CArray[i] = struct_miqt_string(data: families[i], len: csize_t(len(families[i])))

  QFont.init(fcQFont_new10(struct_miqt_array(len: csize_t(len(families)), data: if len(families) == 0: nil else: addr(families_CArray[0])), pointSize, weight))
proc create*(T: type QFont, families: seq[string], pointSize: cint, weight: cint, italic: bool): QFont =

  var families_CArray = newSeq[struct_miqt_string](len(families))
  for i in 0..<len(families):
    families_CArray[i] = struct_miqt_string(data: families[i], len: csize_t(len(families[i])))

  QFont.init(fcQFont_new11(struct_miqt_array(len: csize_t(len(families)), data: if len(families) == 0: nil else: addr(families_CArray[0])), pointSize, weight, italic))
...
proc weight*(self: QFont, ): QFontWeight =

  QFontWeight(fcQFont_weight(self.h))

proc setWeight*(self: QFont, weight: QFontWeight): void =
...
proc resolveMask*(self: QFont, ): cuint =

  fcQFont_resolveMask(self.h)

proc setResolveMask*(self: QFont, mask: cuint): void =

  fcQFont_setResolveMask(self.h, mask)

proc setLegacyWeight*(self: QFont, legacyWeight: cint): void =

  fcQFont_setLegacyWeight(self.h, legacyWeight)

proc legacyWeight*(self: QFont, ): cint =
