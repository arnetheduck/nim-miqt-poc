...
type cQAbstractFileIconProviderVTable = object
  ...
  icon*: proc(vtbl, self: pointer, param1: pointer): pointer {.cdecl, raises: [], gcsafe.}
...
type QAbstractFileIconProvidericonProc* = proc(self: QAbstractFileIconProvider, param1: gen_qfileinfo_types.QFileInfo): gen_qicon_types.QIcon {.raises: [], gcsafe.}
...
proc miqt_exec_callback_cQAbstractFileIconProvider_options(vtbl: pointer, self: pointer): cint {.cdecl.} =
  let vtbl = cast[ptr QAbstractFileIconProviderVTable](vtbl)
  let self = QAbstractFileIconProvider(h: self)
  let virtualReturn = vtbl[].options(self)
  cint(virtualReturn)
...
proc create*(T: type gen_qabstractfileiconprovider_types.QAbstractFileIconProvider,
  ...
  if not isNil(vtbl.icon):
    vtbl[].vtbl.icon = miqt_exec_callback_cQAbstractFileIconProvider_iconWithQFileInfo
