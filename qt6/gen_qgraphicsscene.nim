...
proc miqt_exec_callback_cQGraphicsScene_focusNextPrevChild(vtbl: pointer, self: pointer, next: bool): bool {.cdecl.} =
  let vtbl = cast[ptr QGraphicsSceneVTable](vtbl)
  let self = QGraphicsScene(h: self)
  let slotval1 = next
...
proc create*(T: type gen_qgraphicsscene_types.QGraphicsScene,
  ...
  if not isNil(vtbl.focusNextPrevChild):
    vtbl[].vtbl.focusNextPrevChild = miqt_exec_callback_cQGraphicsScene_focusNextPrevChild
