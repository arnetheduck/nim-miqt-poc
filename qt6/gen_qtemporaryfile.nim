...
type QTemporaryFileopenProc* = proc(self: QTemporaryFile, flags: cint): bool {.raises: [], gcsafe.}
...
proc miqt_exec_callback_cQTemporaryFile_fileName(vtbl: pointer, self: pointer): struct_miqt_string {.cdecl.} =
  let vtbl = cast[ptr QTemporaryFileVTable](vtbl)
  let self = QTemporaryFile(h: self)
  let virtualReturn = vtbl[].fileName(self)
...
proc miqt_exec_callback_cQTemporaryFile_resize(vtbl: pointer, self: pointer, sz: clonglong): bool {.cdecl.} =
  ...
  let self = QTemporaryFile(h: self)
  let slotval1 = sz
  let virtualReturn = vtbl[].resize(self, slotval1)
  virtualReturn
...
proc miqt_exec_callback_cQTemporaryFile_permissions(vtbl: pointer, self: pointer): cint {.cdecl.} =
  let vtbl = cast[ptr QTemporaryFileVTable](vtbl)
  let self = QTemporaryFile(h: self)
  let virtualReturn = vtbl[].permissions(self)
  cint(virtualReturn)
...
proc miqt_exec_callback_cQTemporaryFile_setPermissions(vtbl: pointer, self: pointer, permissionSpec: cint): bool {.cdecl.} =
  ...
  let virtualReturn = vtbl[].setPermissions(self, slotval1)
  virtualReturn
...
proc miqt_exec_callback_cQTemporaryFile_skipData(vtbl: pointer, self: pointer, maxSize: clonglong): clonglong {.cdecl.} =
  let vtbl = cast[ptr QTemporaryFileVTable](vtbl)
  let self = QTemporaryFile(h: self)
...
proc create*(T: type gen_qtemporaryfile_types.QTemporaryFile,
  ...
  if not isNil(vtbl.open):
    vtbl[].vtbl.open = miqt_exec_callback_cQTemporaryFile_openWithFlags
  ...
  if not isNil(vtbl.skipData):
    vtbl[].vtbl.skipData = miqt_exec_callback_cQTemporaryFile_skipData
